...
(function() {
  ...
  var arrToMap = function(langArr) {
    ...
    langArr.forEach(function(lang) {
      if (!window.__KC_LANGUAGES_BASE_MAP__) return;
      map[lang] = window.__KC_LANGUAGES_BASE_MAP__.langToBase[lang];
    });
    return map;
  };
  ...
  script.id = 'langPack';
  var langPath = document.currentScript.src.replace('langLoader_demo', 'locales/' + window.initLang);
  script.src = langPath;

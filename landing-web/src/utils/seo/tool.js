...
import { allLanguages, localeMap } from 'utils/langTools';
import { reduce, split, get } from 'lodash';
import { zElanguages } from './config';
...
export const getLocaleFromLocaleMap = (lang) => {
  ...
  const currentLocale = get(localeMap, lang);
...
const getLangPathFromZendeskLang = (zendeskLang) => {
  ...
  return langPath === window._DEFAULT_LOCALE_ ? '' : langPath;
};
...
const isLangPath = (path) => {
  ...
  languages.forEach((item) => {
    const langPath = _localeMap[item];
    if (langPath) {
...
export const replaceOldOrigin = (content) => {
  ...
  replaceOrigin.forEach((item) => {
    ...
    while (result) {
      ...
      // 线下替换为当前域名，线上保持不变
      const newOrigin = origin === 'net' ? `href="${window.location.origin}` : `${item.newOrigin}${origin}`;

 ...
 */
import { convertPlatformConfig } from 'components/$/CommunityCollect/tools/getConfigByLang';
import { groupConfig } from 'src/test/components/$/CommunityCollect/tools/data/groupConfig';

describe('getConfigByLang.convertPlatformConfig', () => {
  it('参数校验 ok', () => {
    expect(convertPlatformConfig()).toEqual([]);
    expect(convertPlatformConfig(null)).toEqual([]);
    expect(convertPlatformConfig([])).toEqual([]);
  });

  it('正常数据处理 ok', () => {
    const result = convertPlatformConfig(groupConfig);

...
const PosterShare = (props, ref) => {
  ...
  const shareLink = useMemo(
    () => {
      return inviteCode
        ? addLangToPath(`${LANDING_HOST}/eth-merge?rcode=${inviteCode}`)
        : addLangToPath(`${LANDING_HOST}/eth-merge`);
    },
    [inviteCode],
  );
  const isMobile = useIsMobile();
  const shareTitle = useMemo(() => {
    const str = _t('iSTbEwQo46pvLC522HuEdY');
        ...
        setTimeout(async () => {
          let texts = [
            ...
            },
            {
              ...
              fontSize: 12,
              fontWeight: '400',
              text: _t('rzUZdawzDwRpEmzPNUmjYk'),
              x: 60,
              y: positionY_bottom,
              lineHeight: 16,
              lineSpace: 0,
          ...
          create({
            ...
            imgsCommon: [
              {
                img: document.getElementById('ethMerge-QRCode-url'),
                x: 316,
                y: 580,
                width: 44,
                height: 44,

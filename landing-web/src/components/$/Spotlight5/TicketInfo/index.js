...
import helpCenter from 'src/utils/helpCenter';
...
export default class TicketInfo extends React.Component {
  ...
  renderRowNum = (_n, index) => {
    ...
    return (
      <div>{_tHTML('spotlight.lottery.num', { n })}</div>
    );
  };
  ...
  render() {
    ...
    return (
      <div className={style.ticket}>
        <GradientCard className={style.ticketBoxWrap}>
          <div className={style.ticketBox}>
            {(!isSpecialRound) && (
              <div className={style.title}>
                {_tHTML('spotlight.lottery.totalnum', { n: numStr })}
                {<div className={style.seeRule} onClick={this.onRuleClick}>{_t('spotlight5.info.rules')}</div>}

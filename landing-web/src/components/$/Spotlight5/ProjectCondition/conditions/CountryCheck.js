...
import { regionOptions } from 'meta/countries';
...
export default class CountryCheck extends React.Component {
  ...
  render() {
    const {
      isLogin, isCN, rule, isKycCountryInBlackList,
      isCompletedKyc, isNewSpotlight6, children,
    } = this.props;
                  ...
                  <React.Fragment>
                    {isKycCountryInBlackList ? (
                      <div className={style.redtip} key="country-red">
                        <Icon type="close-circle" theme="filled" />{_t('spotlight.nocondition')}
                      </div>
                    ) : (
                      <div className={style.greentip} key="country-green">
                        <Icon type="check-circle" theme="filled" />{_t('spotlight.condition')}
                      </div>
                    )}
                  </React.Fragment>
                ) : (

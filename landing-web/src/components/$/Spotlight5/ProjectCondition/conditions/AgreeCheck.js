...
@connect((state) => {
  return {
    ...
    loading: state.loading.effects['spotlight5/signAgreement'],
  };
})
export default class AgreeCheck extends React.Component {
  ...
  visitModal = () => {
    ...
    sensors.trackClick(['buy', '1'], { coin: rule.currency });
  };
  ...
  render() {
    const { agreement, loading, isLogin, isSignedAgreement, children } = this.props;

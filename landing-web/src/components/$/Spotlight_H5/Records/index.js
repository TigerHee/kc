...
export default class Records extends React.Component {
  ...
  createColumnsMap = () => {
    const map = {};
    const arrs = this.createColumns();
    _.each(arrs, ({ key, ...rest }) => {
      map[key] = {
        title: rest.title,
        render: (record) => {
          if (typeof rest.render !== 'function') {
            return record[rest.dataIndex];
          }
          return rest.render(record[rest.dataIndex], record);
        },
      };
    });
    return map;
    ...
    if (!dataSource.length) {
      return (
        <div className={styles.empty}>
          {_t('spotlight.table.empty')}
        </div>
      );
    }

    const colsMap = this.createColumnsMap();

    return (
      <div className={styles.list}>
        {_.map(dataSource, (record) => {
          return (
            <div key={record.id} className={styles.item}>
              <div className={styles.head}>{colsMap['currency'].render(record)}</div>
              <div className={styles.row}>
                <div className={styles.col}>
                  <div className={styles.title}>{colsMap['id'].title}</div>
                  <div className={styles.content}>{colsMap['id'].render(record)}</div>
                </div>
                <div className={styles.col}>
                  <div className={styles.title}>{colsMap['createdAt'].title}</div>
                  <div className={styles.content}>{colsMap['createdAt'].render(record)}</div>
                </div>
                <div className={styles.col}>
                  <div className={styles.title}>{colsMap['price'].title}</div>
                  <div className={styles.content}>{colsMap['price'].render(record)}</div>
                </div>
              </div>
              <div className={styles.row}>
                <div className={styles.col}>
                  <div className={styles.title}>{colsMap['amount'].title}</div>
                  <div className={styles.content}>{colsMap['amount'].render(record)}</div>
                </div>
                <div className={styles.col}>
                  <div className={styles.title}>{colsMap['total'].title}</div>
                  <div className={styles.content}>{colsMap['total'].render(record)}</div>
                </div>
                <div className={styles.col}>
                  <div className={styles.title}>{colsMap['status'].title}</div>
                  <div className={styles.content}>{colsMap['status'].render(record)}</div>
                </div>
              </div>
            </div>
          );

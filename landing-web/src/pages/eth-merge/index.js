...
import HomePage from 'components/$/EthMerge/HomePage';
import PosterShare from 'components/$/EthMerge/PosterShare';

import { LANDING_HOST } from 'utils/siteConfig';
import 'animate.css';
import Header from 'src/components/$/EthMerge/HomePage/Header';
...
const IndexPage = () => {
  ...
  const shareLink = useMemo(
    () => {
      return inviteCode
        ? addLangToPath(`${landUrlHost}/eth-merge?rcode=${inviteCode}`)
        : addLangToPath(`${landUrlHost}/eth-merge`);
    },
  ...
  useEffect(
    () => {
      dispatch({
        type: 'ethMerge/pullMarketInfo',
      });
      dispatch({
        type: 'ethMerge/getConfig',
      });
      dispatch({ type: 'ethMerge/getPostDetail' });
      dispatch({ type: 'ethMerge/getPoolStakingProducts' });
    },
    [dispatch],
  );

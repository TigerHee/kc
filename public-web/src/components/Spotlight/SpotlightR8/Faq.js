...
import question from 'static/spotlight8/question.svg';
import questionLight from 'static/spotlight8/questionLight.svg';
...
const Faq = () => {
  const { faqModule = [] } = useSelector((state) => state.spotlight8.detailInfo, shallowEqual);
  ...
  return !faqModule?.length ? null : (
    <Wrapper>
      ...
      <Title
        title={_t('newhomepage.faq')}
        icon={currentTheme === 'dark' ? question : questionLight}
      />

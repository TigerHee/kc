...
import { Dropdown, isPropValid, styled, useTheme } from '@kufox/mui';
  ...
  const OverLay = (
    <OverlayContent inDrawer={inDrawer} isIN theme={theme}>
      ...
      <div className="activeItem">
        <ConfirmOutlined size="16" />
        {currentLangItem.langName}
      </div>
      <React.Fragment>
        {_.map(getLangList(), (item, key) => {
          return (
            <div
  ...
  const OverLayInDrawer = (
    <OverlayContent inDrawer={inDrawer} theme={theme} className="inDrawer">
      <p className="title inDrawer">
        {_t('language')}
        <img src={CloseIcon} onClick={onClose} className="closeIcon" alt="" />
      </p>

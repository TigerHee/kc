 ...
 */
import { Input, Spin } from '@kufox/mui';
...
class InputPwdBox extends React.Component {
  ...
  render() {
     ...
     * @returns {any[]}
     */
    const createIpts = (_len, type, isPassword = true) => {
      return new Array(_len).fill(1).map((key, idx) => {
        const ipt = (
          <Input
            ...
            value={vals[idx] ? (isPassword ? '*' : vals[idx]) : undefined}
            cvalue={vals[idx]}
    ...
    return (
      <Spin spinning={loading}>
        <div className={style.ipt_pwd_wrapper}>
          {createIpts(len, this.props.type, this.props.isPassword)}
        </div>
      </Spin>
    );
  }
...
InputPwdBox.propTypes = {
  ...
  isPassword: PropTypes.bool, // 密文/明文
};

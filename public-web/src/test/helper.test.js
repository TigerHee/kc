...
describe('transformRtlNum', () => {
  it('should return value with char in LTR when isRTL is false', () => {
    const result = transformRtlNum({ char: '%', value: '50', isPositive: false, lang: 'en' });
    expect(result).toBe('50%');
  });
  it('should add negative sign for negative values', () => {
    const result = transformRtlNum({ char: '%', value: '-50', isPositive: false, lang: 'en' });
    expect(result).toBe('-50%');
  });

  it('should handle non-numeric values correctly', () => {
    const result = transformRtlNum({ char: '%', value: 'abc', isPositive: false, lang: 'en' });
    expect(result).toBe('abc%');
  });
});

describe('formatLocalLangNumber', () => {
  it('should format number correctly for en_US locale', () => {
    const result = formatLocalLangNumber({ lang: 'en_US', data: 1234.567 });
    expect(result).toBe('1,234.567');
  });

  it('should format number correctly for de_DE locale', () => {
    const result = formatLocalLangNumber({ lang: 'de_DE', data: 1234.567 });
    expect(result).toBe('1.234,567');
  });

  it('should handle null data correctly', () => {
    const result = formatLocalLangNumber({ lang: 'en_US', data: null });
    expect(result).toBe(null);
  });

  it('should apply interceptDigits correctly', () => {
    const result = formatLocalLangNumber({ lang: 'en_US', data: 1234.56789, interceptDigits: 2 });
    expect(result).toBe('1,234.56');
  });

  it('should apply options correctly', () => {
    const result = formatLocalLangNumber({
      lang: 'en_US',
      data: 1234.567,
...
describe('gtagUtils', () => {
  ...
  describe('gtag_report_conversion', () => {
    it('应该调用gtag和ym函数', () => {
      gtag_report_conversion();
    ...
    it('当不提供URL时，回调函数不应该改变location', () => {
      const originalLocation = window.location.href;
      gtag_report_conversion();

      // 获取回调函数
      const callback = window.gtag.mock.calls[0][2].event_callback;
      callback();
    ...
    it('应该返回false', () => {
      const result = gtag_report_conversion();
  ...
  describe('getCmsCdnHost', () => {
    it('应该返回正确的CDN主机地址', () => {
      const result = getCmsCdnHost();
      expect(result).toBe('https://assets.staticimg.com/cms-static');
    });
  });
  ...
  describe('loadImg', () => {
    ...
    beforeEach(() => {
      imageInstances = [];
      mockImage = jest.fn().mockImplementation(() => {
        const instance = {
          src: '',
          onload: null,
          onerror: null,
    ...
    it('should load a single image and call callback', (done) => {
      const imgUrl = 'test1.jpg';
      const callback = jest.fn(() => {
        expect(callback).toHaveBeenCalledTimes(1);
        expect(mockImage).toHaveBeenCalledTimes(1);
        expect(imageInstances[0].src).toBe(imgUrl);
        done();
      });
    ...
    it('should load multiple images sequentially and call callback once', (done) => {
      const imgUrls = ['test1.jpg', 'test2.jpg', 'test3.jpg'];
      const callback = jest.fn(() => {
        expect(callback).toHaveBeenCalledTimes(1);
        expect(mockImage).toHaveBeenCalledTimes(2);
        expect(imageInstances[0].src).toBe(imgUrls[0]);
        expect(imageInstances[1].src).toBe(imgUrls[1]);
        done();
      });

      loadImg(imgUrls, callback);
    ...
    it('should not call callback if no images are provided', () => {
      const callback = jest.fn();
      loadImg([], callback);
    ...
    it('should handle single image without callback', () => {
      const imgUrl = 'test.jpg';
      expect(() => {
        loadImg([imgUrl]);
        imageInstances[0].onload();
  ...
  describe('numSeparateDecimal', () => {
    // Test invalid inputs
    it('should return 0 for null/undefined/NaN/empty values', () => {
      expect(numSeparateDecimal(null)).toBe(0);
      expect(numSeparateDecimal(undefined)).toBe(0);
      expect(numSeparateDecimal(NaN)).toBe(0);
      expect(numSeparateDecimal('')).toBe(0);
      expect(numSeparateDecimal(0)).toBe(0);
      expect(numSeparateDecimal(false)).toBe(0);
    });

    // Test integers
    it('should return same number for integers', () => {
      expect(numSeparateDecimal(123)).toBe(123);
      expect(numSeparateDecimal(-456)).toBe(-456);
      expect(numSeparateDecimal('789')).toBe(789);
    });

    // Test decimal truncation
    it('should truncate decimals to specified digits', () => {
      expect(numSeparateDecimal(123.456789, 2)).toBe(123.45);
      expect(numSeparateDecimal(123.456789, 3)).toBe(123.456);
      expect(numSeparateDecimal(-123.456789, 1)).toBe(-123.4);
      expect(numSeparateDecimal('123.456789', 4)).toBe(123.4567);
    });

    // Test when decimals are shorter than interceptDigits
    it('should return original number when decimals are shorter than interceptDigits', () => {
      expect(numSeparateDecimal(123.4, 2)).toBe(123.4);
      expect(numSeparateDecimal(123.45, 3)).toBe(123.45);
      expect(numSeparateDecimal('123.456', 5)).toBe(123.456);
    });

    // Test edge cases
    it('should handle edge cases correctly', () => {
      // Default interceptDigits is 2
      expect(numSeparateDecimal(123.456)).toBe(123.45);
      
      // Zero interceptDigits
      expect(numSeparateDecimal(123.456, 0)).toBe(123);
      
      // Very large interceptDigits
      expect(numSeparateDecimal(123.456789, 10)).toBe(123.456789);
      
      // Scientific notation
      expect(numSeparateDecimal('1.2345e-2', 3)).toBe(1.234);
    });

    // Test string inputs
    it('should handle string inputs correctly', () => {
      expect(numSeparateDecimal('123.456', 1)).toBe(123.4);
      expect(numSeparateDecimal('123', 2)).toBe(123);
      expect(numSeparateDecimal('abc', 2)).toBe(NaN); // Invalid number string
    });
  });
});

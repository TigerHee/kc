...
describe(`【${url}】`, () => {
  ...
  it(`${url} route test`, () => {
    ...
    // 点击 broker tab
    cy.get('[data-inspector="api_create_other_tab"]').should('exist').click();
    // broker 选择存在
    cy.get('[data-inspector="api_create_broker_select"]').should('exist');
    cy.get('[data-inspector="api_create_api_name"]').should('exist');
    cy.get('[data-inspector="api_create_api_password"]').should('exist');
    cy.get('[data-inspector="api_create_auth_radio"]').should('exist');
    // ip 限制不存在
    cy.get('[data-inspector="api_create_api_ip_nolimit"]').should('not.exist');

    // 点击 api 交易 tab
    cy.get('[data-inspector="api_create_api_tab"]').should('exist').click();
    // broker 选择不存在
    ...
    // // api security 页面
    // cy.get('[data-inspector="api_create_security_page"]').should('exist');
  });
});

checkLeadTradeApiUrl(true);

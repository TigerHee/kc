...
export async function queryTradeAccount(baseAmount, baseCurrency) {
  return pull(
    `${ACCOUNT_FRONT}/query/trade-account?baseAmount=${baseAmount || 0}&baseCurrency=${
      baseCurrency || 'BTC'
    }`,
  );
}
 ...
 * @returns {Object}
 */
export async function exportMainAccountDetail({ bizType, currency, direction, endAt, startAt }) {
  const query = {
    bizType,
    currency,
    direction,
 ...
 * @returns {Object}
 */
export async function exportTradeAccountDetail({ bizType, currency, direction, endAt, startAt }) {
  const query = {
    bizType,
    currency,
    direction,
 ...
 * @returns {Object}
 */
export async function queryTotalBalance({ baseCurrency = window._BASE_CURRENCY_ }) {
...
export async function getMainAccountAssets(query) {
  return pull('/asset-front/v2/asset/main-account-assets', query);
}
// 查询用户杠杆账户资产

...
import { goVerify, goVerifyWithAddress, goVerifyWithToken } from '@kucoin-gbiz-next/verification';
...
export default function ResetSecurity({ token, address, onRefreshToken }) {
  ...
  const handleBack = () => {
    if (
      [RESET_SECURITY_SCENES.SELECT_ITEMS, RESET_SECURITY_SCENES.RESULT_VERIFYING].includes(scene)
    ) {
      // 选择页面及审核中页面，直接退出
      handleExist();
    } else {

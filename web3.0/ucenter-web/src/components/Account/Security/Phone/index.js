...
import { PhoneAreaSelector, SMSVerifyInput } from '@kucoin-gbiz-next/entrance';
  ...
  const handleSendCode = (currentSendChannel = 'SMS') => {
    form.validateFields(['countryCode', 'phone']).then(async ({ countryCode, phone }) => {
      ...
      const request = customSendRequest
        ...
        : dispatch({
            type: isBind ? 'account_security/sendBindCode' : 'account_security/sendPhoneCode',
            payload: isBind
              ? {
                  type: 'phone',
                  params: {
                    bizType: 'BIND_PHONE_V2',
                    phone,
                    countryCode,
              ...
              : {
                  ...
                  isVoice: currentSendChannel === 'VOICE',
                },
          });
      request

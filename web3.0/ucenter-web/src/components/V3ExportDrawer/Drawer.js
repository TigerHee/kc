...
import { MailAuthorize } from '@kucoin-biz/common-base';
...
export default React.memo(({ showRecord = true, isInDrawer = true }) => {
    ...
    validateFields()
      .then(async (values) => {
        ...
        const bizTypes = paramsCodes.map((item) => {
          return { code: item, name: ALL_TOP_CODES_MAP[item] };
        });
        // 月结账单
          ...
          if (!isAccountStatement) {
            ...
            if (isOverTwoYears && +data > 350) {
              setConfirmDialogConfig({
                open: true,
                onCancel: () => {
                  setConfirmDialogConfig((i) => ({ ...i, open: false }));
                },
                okText: null,
  ...
  useEffect(() => {
    ...
    if (!isSub) {
      dispatch({ type: 'order_meta/fetchSubUserList' });
    }
  }, [dispatch, isSub]);
      ...
      <ContentWrapper data-inspector="bill_export_drawer">
        <ContentBody>
          <Form form={form} layout="horizontal">
            <FormItem name="codes" rules={[{ required: true, message: '' }]}>
              ...
              !isAccountStatement ? (
                ...
                <SubAccountAccordion>
                  ...
                  >
                    ...
                    <span>
                      {_t('0ea8a94d8a704000ad6c', { count: (subAccountListValue || []).length })}
                    </span>
                  </SubAccountAccordionHeader>

...
async function appPerformanceChange(rootElement, location) {
  ...
  let nextApp = apps.find((o) => {
    if ('activeSiteConfig' in o && 'activeBrandKeys' in o) {
      console.error(
        `registerApplication activeSiteConfig and activeBrandKeys should not be used at the same time`,
      );
    }

    // 如果配置了 activeSiteConfig，且为 false，则不匹配
    if (o.activeSiteConfig && !o.activeSiteConfig()) {
      return false;
    }
  ...
  if (nextApp !== activeApp) {
    ...
    if (window._useSSG) {
      preloadComponent(appContext.routes, appContext.history.location.pathname).then(function () {
        ReactDOM.hydrate(<App app={appContext} appName={activeApp.name} />, rootElement, () => {
      ...
      ReactDOM.render(<App app={appContext} appName={activeApp.name} />, rootElement, () => {
        const hydrateEnd = performance.now();
  ...
  const baseRoutes = [
    {
      path: '*',
      component: () => (
        <Redirect to={`/404?from=${encodeURIComponent(window.location.pathname)}`} />
      ),
    },
  ];

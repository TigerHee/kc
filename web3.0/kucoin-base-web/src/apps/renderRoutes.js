  ...
  return opts.routes ? (
    <Switch {...switchProps}>
      ...
      {/* 
        ...
        比如 /ucenter/signin 路由，假设在泰国站被屏蔽了，但是用户访问 /ucenter/signin 的时候，还是会进入到 ucenter-web 项目中，只是会渲染空白内容。
        这时候通过下面的 404 配置，就可以重定向到 404 页面
        2. 多租户场景下，应用被屏蔽了，则应该自行跳转到 404 页面。
        比如 ucenter-web 整个应用被屏蔽，讲道理应该进入兜底的 kucoin-main-web 项目。但现在的 mainWebExcludePaths 配置导致 ucenter-web 项目的路由访问，
        进不去 kucoin-main-web 项目。此时就可以通过下面的 404 配置，重定向到 404 页面 
      */}

      {/* 有些站点自定义了 basename，比如 /zh-hant/futures，这时候就会进入 /zh-hant/futures/404 页面，所以不能用这种方法了，而是换成下面的 location.href 跳转 */}
      {/* <Redirect to="/404" /> */}
      <Route
        path="*"
        render={() => {
          window.location.href = `${basename}/404?from=${encodeURIComponent(

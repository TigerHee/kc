...
import { SlickList, SlickSlider } from './StyledComps';
...
export class InnerSlider extends React.Component {
  ...
  componentDidMount = () => {
    ...
    document.querySelectorAll &&
      Array.prototype.forEach.call(document.querySelectorAll('.kux-slick-slide'), (slide) => {
  ...
  checkImagesLoad = () => {
    const images =
      (this.list &&
        ...
        this.list.querySelectorAll('.kux-slick-slide img')) ||
      [];
  ...
  changeSlide = (options, dontAnimate = false) => {
    ...
    if (this.props.focusOnSelect) {
      const nodes = this.list.querySelectorAll('.kux-slick-current');
  ...
  swipeEnd = (e) => {
    ...
    if (!state) return;
    const { triggerSlideHandler } = state;
  ...
  render = () => {
    const className = classnames('kux-slick-slider', this.props.className, {
      'kux-slick-vertical': this.props.vertical,
    });
    const spec = { ...this.props, ...this.state };
    ...
    if (this.props.unslick) {
      listProps = { className: 'kux-slick-list' };
      innerSliderProps = { className };
    }
    return (
      <SlickSlider {...innerSliderProps}>
        {!this.props.unslick ? prevArrow : ''}
        <SlickList ref={this.listRefHandler} {...listProps}>

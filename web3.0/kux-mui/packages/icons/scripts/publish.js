...
async function publish() {
  console.log('Prepare to publish @kux/icons...');
  ...
  try {
    ...
    console.log('Publish @kux/icons success.');
  } catch (e) {
    spinner.stop();
    console.log('e:', e);
    console.log('Publish @kux/icons failed.');
    process.exit(0);
  }

  try {
    const tagName = `@kux/icons@${version}`;
  ...
  spinner.stop();
  console.log('Publish @kux/icons completed.');
  process.exit(0);
}

...
function dirPath() {
  const IS_DEV_ENV = process.env.NODE_ENV === 'dev' || process.env.UMI_ENV === 'dev' || process.env.BUILD_ENV === 'dev';
  return IS_DEV_ENV ? `cdnAssets` : `dist/${pkg.name}/${pkg.version}`;
}

compressLottieFiles([path.resolve(cwd, `${dirPath()}/static/securityV2/lottie`)]);

...
export default class NumberRemainBox extends React.Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (number !== prevState._number) {
      const newNums =
        direction === NumberRemainBox.UP
          ? [prevState.nums[1], number]
          : [number, prevState.nums[0]];
      return {
        nums: newNums,
        _number: number,
      };
  ...
  render() {
    ...
    return (
      <div className={`${style.outter} ${className}`} data-role="number-remain-outter">
        <div
          key={`in-${number}`}
          className={`${style.inner} ${innerClassName}`}
          data-role="number-remain-inner"
        >
          {_.map(nums, (num) => {
            return (
              <div key={`${num}`} className={style.number}>
                {num}
              </div>
            );
          })}

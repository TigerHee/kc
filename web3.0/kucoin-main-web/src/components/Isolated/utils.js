 ...
 * @Description: 逐仓的工具函数
 */
import { each, isNil, uniq } from 'lodash';
import {
  add,
  sub,
  Decimal,
...
export const getEarning = (netAsset, accumulatedPrincipal, pnlPrecision) => {
  if (isNil(accumulatedPrincipal)) return null;
  let result;
  try {
    result = normalizeNumber(sub(netAsset, accumulatedPrincipal), pnlPrecision || 2);
  } catch (e) {
    result = null;
  }
  return result;
...
export const getLeveragePoint = (maxLeverage, result = [1]) => {
  ...
  if (maxLeverage <= pointCount) {
    ...
    return [...Array.from(new Array(Math.floor(maxLeverage))).map((v, i) => i + 1), maxLeverage];
  }

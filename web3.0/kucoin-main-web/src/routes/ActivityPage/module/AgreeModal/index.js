...
import { Modal, Checkbox, Button } from '@kc/ui';
          ...
          {!hideAgreeButton && (
            <React.Fragment>
              <div className="pt-12 pb-12 text-center">
                <Checkbox checked={checked} onChange={this.checkChange} className={style.checkbox}>
                  {_t('spotlight.agreemodal.check')}
                </Checkbox>
              </div>
              <div className={style.button}>
                <Button type="primary" disabled={!checked} loading={loading} onClick={onAgree}>
                  {_t('spotlight.ok')}
                </Button>
              </div>
            </React.Fragment>

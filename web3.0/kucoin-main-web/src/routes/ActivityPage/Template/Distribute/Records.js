...
@connect((state) => {
  return {
    ...
    records: state.distribute.list,
    loading: state.loading.effects['distribute/getList'],
  };
})
@requireProps({
  user(value) {
...
export default class Records extends React.Component {
  ...
  componentDidMount() {
    ...
    if (isLogin) {
      dispatch({ type: 'distribute/getList@polling' });
    } else {
      nologin();
    }
  }
  ...
  componentWillUnmount() {
    ...
    dispatch({ type: 'distribute/getList@polling:cancel' });
  }
  ...
  createColumns = () => {
    return [
      ...
      {
        title: `${_t('spotlight.table.price')} ${this.props.quoteCurrency}`,
        key: 'price',
        dataIndex: 'price',
        render(item, record) {
          ...
          return (
            <div>
              <CoinPrecision value={item || 0} coin={quoteCurrency} />
            </div>
          );
        },
      },
      {
        title: `${_t('spotlight.table.amount')} ${this.props.currency}`,
        key: 'amount',
        dataIndex: 'amount',
        render(item, record) {
          ...
          return (
            <div>
              <CoinPrecision value={item} coin={currency} />
            </div>
          );
        },
      },
  ...
  render() {
    ...
    return (
      <GradientRecords
        styles={{ height: '300px', margin: '80px 56px 46px 56px' }}
        panelStyle={{
          minHeight: '400px',
          backgroundImage: 'linear-gradient( 134deg,#2a0b6d 10%, #102683 100%, #0f7dff 100% )',
        }}
        rowKey="id"

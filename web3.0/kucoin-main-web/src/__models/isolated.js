 ...
 * @Description:逐仓相关
 */
import { each, isEqual, pick, values, map, filter, isNil } from 'lodash';
...
export default extend(baseUtil, polling, {
  ...
  effects: {
    *pullIsolatedAppoint({ forceFetch = false }, { call, put }) {
      ...
      const { kcWs: socket, Topic, TOPIC_STATE } = yield import('src/utils/socket');
      if (!forceFetch && socket.connected()) {
        const topic = Topic.get('/margin/isolatedPosition:{SYMBOL_LIST}', {
          SYMBOLS: ['all'],
        });
        const topicStateData = TOPIC_STATE.SUBSCRIBED;
        if (socket) {
          const topicState = socket.getTopicState();
    ...
    *updateIsolatedAccountAssets({ payload }, { put, select }) {
      ...
      if (!isNil(totalMarginBalance)) {
        nextIsolatedAccountAssets.displayTotalMarginBalance = isBTC
          ? totalMarginBalance
          : btcAmount2CurrencyAmount(totalMarginBalance, baseCurrencyMarkPrice);
      }
      if ([totalBalance, totalLiability].every((v) => !isNil(v))) {
        // 净资产
      ...
      // 今日盈亏
      if (!isNil(todayPnlAmount)) {
        nextIsolatedAccountAssets.todayPnlAmount = todayPnlAmount;
        nextIsolatedAccountAssets.displayTodayPnlAmount = isBTC
          ? todayPnlAmount
          : btcAmount2CurrencyAmount(todayPnlAmount, baseCurrencyMarkPrice);
      }
      if (!isNil(todayPnlRatio)) {
        nextIsolatedAccountAssets.todayPnlRatio = todayPnlRatio;
        nextIsolatedAccountAssets.displayTodayPnlRatio = todayPnlRatio;
      }
      yield put({
        type: 'update',
        payload: {
    ...
    *updatePosition({ payload, isRest = false }, { put, select }) {
      ...
      each(values(payload), (item) => {
        ...
        const { flDebtRatio = 0, pnlPrecision = 2 } = isolatedSymbolsMap[tag] || {};
        ...
        // 盈亏
        const earning = getEarning(netAsset, accumulatedPrincipal, pnlPrecision);
        // 盈利率
    ...
    *pullTargetPrice({ put, call }) {
      ...
      const topicStateData = TOPIC_STATE.SUBSCRIBED;
      if (connected) {
        const topic = Topic.get('/indicator/markPrice:{SYMBOL_LIST}', {
          SYMBOLS: [symbolsStr],
        });

...
import { captchaInit, captchaValidate, getImageVer } from 'services/ucenter/captcha';
...
export default extend(base, {
  ...
  effects: {
    *captcha_start({ type, payload: { bizType = '' } }, { put }) {
    ...
    *captcha_init({ payload = {}, type, onCallBack }, { call, put, select, race, take }) {
      ...
      const { success } = yield race({
        success: take('captcha_verifyIfOk/@@end'),
      });
      if (success) {
    ...
    *captcha_retry({ payload: { bizType = '' } }, { put }) {
    ...
    *captcha_verifyIfOk({ type, payload }, { call, select, put }) {
      const modelName = type.split('/')[0];
      // 图片验证需要这个参数
      const { challenge, captcha_gt } = yield select((state) => state.captcha);

...
export const genWSCalibration = () => {
  const wsCalibration = {
    ...
    _speed: 0,
    getSpeed: () => {
      return wsCalibration._speed;
    },
    speed: (len) => {
      const t1 = Date.now();
      if (wsCalibration._t0 > 0) {
        wsCalibration._speed = len / (t1 - wsCalibration._t0); // q / ms
        // console.log('speed', wsCalibration._speed);
      }
      wsCalibration._t0 = t1;
    },
  };
 ...
 * @param {*} limit
 */
export const genDataQueue = (limit) => {
  const sliceLimit = -1 * limit;
  let save = [];

  function saveItems(item) {
    save.push(item);
    save = save.slice(sliceLimit);
  }

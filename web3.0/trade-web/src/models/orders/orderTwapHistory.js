...
const generateQueryStatusList = (status) => {
  if (status) {
    return [status];
  }
  return [TWAP_PROCESS_STATUS.COMPLETED, TWAP_PROCESS_STATUS.CANCELLED];
};
...
export default extend(base, polling, filter, clearRecords, {
  ...
  effects: {
    *query(_, { select, call, put }) {
      ...
      const { symbol, side, status, ...others } = filters;
      ...
      const ret = yield call(queryTWAPOrders, {
        ...
        side,
        statusList: generateQueryStatusList(status),
      });

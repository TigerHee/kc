...
// 保证金余额
export const marginBalanceOperator = (availableBalance, orderMargin, maintMargin, withdrawHold) => {
  return plus(plus(plus(availableBalance)(orderMargin))(maintMargin))(withdrawHold);
};
...
// 开仓手续费
export const openFeeOperator = (addSize, minDealPrice, takerFeeRate, fixTakerFee) => {
  const roundPrice = toFixed(dividedBy(1)(minDealPrice))(10);
  const roundValue = abs(toFixed(multiply(roundPrice)(addSize))(8));
  return plus(multiply(roundValue)(takerFeeRate))(abs(multiply(addSize)(fixTakerFee)));
};

// 平仓手续费
export const closeFeeOperator = (addSize, minDealPrice, takerFeeRate, fixTakerFee, leverage) => {
  const roundPrice = toFixed(dividedBy(1)(minDealPrice))(10);
  const roundValue = abs(toFixed(multiply(roundPrice)(addSize))(8));
  return plus(multiply(multiply(roundValue)(plus(dividedBy(1)(leverage))(1)))(takerFeeRate))(
...
export const posIncreaseOddsOperator = (positionSize, orderSize) => {
  return equals(positionSize)(0) || greaterThan(abs(plus(orderSize)(positionSize)))(abs(orderSize));
};
...
export const toDP = (a) => {
  a = transformParam(a);
  return (b, mod = Decimal.ROUND_HALF_CEIL) => {
    if (b == null) {
      return a.toDP();
    }
    return a.toDP(b, mod);
...
export const forwardStopPercentOperator = ({
  ...
  return toFixed(multiply(toFixed(dividedBy(stopLossAmount)(realAmount))())(100))();
};
// 反向止损比例
...
export const stopLossTipOperator = ({ stopPercent, lev }) => {
  return toFixed(multiply(dividedBy(minus(1)(abs(dividedBy(stopPercent)(100))))(lev))(100))();
};
...
export const toNearest = (a) => {
  a = transformParam(a);
  return (b, mod = Decimal.ROUND_UP) => {
    b = transformParam(b);
    return a.toNearest(b, mod);
  };
};

...
export default extend(baseUtil, polling, {
  ...
  effects: {
    ...
    *userSignAgreement({ payload = {} }, { put, call }) {
      const { success } = yield call(userSignAgreement);
      if (success) {
        yield put({
    ...
    *pullUserMarginPostionDetail({ payload = {} }, { put, call, select }) {
      ...
      if (margin.length && connected) {
        const topics = ['/margin/account', '/margin/position'];
    ...
    *updateDebtRatio({ payload = {} }, { put, select }) {
      ...
      const banchMarginLiability = [];
      forIn(debtList, (value, key) => {
        if (marginMap[key] && marginMap[key].liability !== value) {
          banchMarginLiability.push({ currency: key, liability: value });
        }
      });
      if (typeof debtRatio === 'number') {
        if (banchMarginLiability.length) {
          yield put({
            type: 'user_assets/updateCrossLiability',
            payload: banchMarginLiability,
          });
        }
        yield put({
          type: 'update',
    ...
    *registerCrossAccountPolling(__, { put }) {
      yield put({
        type: 'watchPolling',
        payload: { effect: 'pullUserMarginPostionDetail', interval: 8 * 1000 },
      });
    },
    *updateMarginModalConfig({ payload = {} }, { put, select }) {

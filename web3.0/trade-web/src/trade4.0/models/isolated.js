 ...
 * @Description:逐仓相关
 */
import { each, isEqual, pick, isEmpty } from 'lodash';
...
export default extend(baseUtil, polling, {
  ...
  state: {
    ...
    leverageModalConfig: {}, // 设置杠杆倍数弹窗
    cancelClosePositionModalConfig: {},
  ...
  reducers: {
    ...
    updateClosePositionModalConfig(state, { payload }) {
      return {
        ...state,
        closePositionModalConfig: { ...state.closePositionModalConfig, ...payload },
      };
    },
    updateCancelClosePositionModalConfig(state, { payload }) {
      return {
        ...state,
        cancelClosePositionModalConfig: { ...state.cancelClosePositionModalConfig, ...payload },
      };
    },
    ...
    changeUserLeverage(state, { payload }) {
      const { userLeverage, tag } = payload;
      return {
        ...state,
        positionMap: {
          ...state.positionMap,
          [tag]: { ...state.positionMap[tag] || {}, userLeverage },
        },
      };
    },
  },
  effects: {
    ...
    *pullTargetPrice({ payload = {} }, { put, call, select }) {
      ...
      // sokect正常连接并且topic_state为1时，阻止此次fetch
      const connected = yield workerSocket.connected();
      if (!isEmpty(targetPriceMap) && connected) {
      ...
      if (success) {
        const _targetPriceMap = {};
        each(data, (item) => {
          _targetPriceMap[item.symbol] = item.value;
        });
        yield put({
          type: 'updateTargetPrice',
          payload: _targetPriceMap,
        });
      }
    },
    ...
    *pullAssetsList({ payload = {} }, { call, put, select }) {
      const { currentSymbol } = yield select((state) => state.trade);
      const isSpotSymbol = isSpotTypeSymbol(currentSymbol);
      if (!isSpotSymbol) {
        return;
      }
      const { data } = yield call(serv.getIsolatedAssetsList, {
    ...
    *registerTargetPricePolling(__, { put }) {
      yield put({
        type: 'watchPolling',
        payload: { effect: 'pullTargetPrice' },
      });
    },
    *registerIsolatedAccountPolling(__, { put }) {
      yield put({
        type: 'watchPolling',
        payload: { effect: 'pullIsolatedAppoint' },
      });
    },
    *oneClickLiquidation({ payload = {} }, { call }) {
      const res = yield call(serv.oneClickLiquidation, payload);
      return res;
    },
    *cancelOneClickLiquidation({ payload = {} }, { call }) {
      const res = yield call(serv.cancelOneClickLiquidation, payload);
      return res;
    },
    *updateLeverageModalConfig({ payload = {} }, { put, select }) {
      const { leverageModalConfig } = yield select(state => state.isolated);

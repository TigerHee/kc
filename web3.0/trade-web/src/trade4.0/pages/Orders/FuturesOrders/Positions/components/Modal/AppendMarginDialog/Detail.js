...
import useAvailableBalance from 'src/trade4.0/hooks/futures/useAvailableBalance';
...
const Detail = (props) => {
  const {
    ...
    symbol,
    isTrialFunds,
  } = useSelector((state) => state.futures_orders.appendMarginDetail, isEqual);
  ...
  return (
    <DetailBox>
      ...
      <DetailItem>
        ...
        <DetailItemContent>{`${(+leverage).toFixed(2)} x`}</DetailItemContent>
      </DetailItem>
      <DetailItem>

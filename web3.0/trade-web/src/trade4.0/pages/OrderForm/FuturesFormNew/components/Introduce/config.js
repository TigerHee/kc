...
import HiddenH5 from '@/assets/futures/hidden-order-h5.png';
import Hidden from '@/assets/futures/hidden-order.png';
...
import { _t, _tHTML } from '../../builtinCommon';
...
const TimeWeightedSquareOutlined = styled.div`
  ...
  &.full-grey {
    background-color: ${(props) => props.theme.colors.cover12};
  }
`;
...
export const ORDER_TYPE_INTROS = (isSm) => [
  ...
  // 条件限价
  {
    ...
    content: {
      introduce: _t('futures.orderST.title'), // 类型介绍
      illustration: {
        description: (
          <>
            <DescriptionContent>{_tHTML('futures.orderST.content')}</DescriptionContent>
            <DescriptionSubTitle>{_t('aab8d86366ea4000aa24')}</DescriptionSubTitle>
  ...
  // 条件市价
  {
    ...
    content: {
      introduce: _t('futures.orderSM.title'), // 类型介绍
      illustration: {
        description: (
          <>
            <DescriptionContent>{_tHTML('futures.orderSM.content')}</DescriptionContent>
            <DescriptionSubTitle>{_t('aab8d86366ea4000aa24')}</DescriptionSubTitle>
  ...
  // 隐藏委托
  {
    ...
    content: {
      introduce: _t('futures.orderH.title'), // 类型介绍
      illustration: {
        description: (
          <>
            <DescriptionContent>{_tHTML('trade.tooltip.visible')}</DescriptionContent>
            {/* <DescriptionSubTitle>{_t('aab8d86366ea4000aa24')}</DescriptionSubTitle>
            <DescriptionContent>{_t('9f4ab8c98db44000a094')}</DescriptionContent> */}
          </>
        ),
        img: isSm ? Hidden : HiddenH5,
      },
      timeWeightedImgItems: [
        // 图例
        {
          itemIcon: <TimeWeightedAreaGradient className="primary" />,
          itemTitle: _t('trade.order.displayQty'),
        },
        {
          itemIcon: <TimeWeightedAreaGradient className="full-grey" />,
          itemTitle: _t('futures.orderH.amount'),
        },
      ],
    },
  },

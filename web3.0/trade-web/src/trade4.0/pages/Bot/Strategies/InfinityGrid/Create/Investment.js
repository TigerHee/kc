...
import { maxInverstConfig } from '../config';

/**
 * @description: 创建/运行中修改投资额会用到
 * @return {*}
 */
export default ({ buySellNum = {}, symbolInfo = {}, balance = {}, minInvest, form }) => {
  ...
  return (
    <>
      <Investment minInvest={minInvest} maxInvestment={maxInverstConfig[quota]} symbol={symbolCode} hasMultiCoin />
      {!!useBaseCurrency && (
        <>
          ...
          <Flex fs={12} sb lh="130%">
            <LabelPopover
              textProps={{ fs: 12, color: 'text40', lh: '12px' }}

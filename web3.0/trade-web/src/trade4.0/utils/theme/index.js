 ...
 (strArr, ...values) => (props) => {
  try {
    const { isUp = false, condition = false,
      conditionKey = 'coditionVal', negation = false,
    } = conf || {};
 ...
 * 容器响应式处理，不满足条件时样式
 */
export const eNotConditionStyle = (breakVal, conditionKey) => {
  return eResponseHandleFn(breakVal, { condition: true, conditionKey, negation: true });
};

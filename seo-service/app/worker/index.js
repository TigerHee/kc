...
class Worker extends EventEmitter {
  constructor() {
    super();
    this.puppeteerManager = new PuppeteerManager(configs);
    this.messenger = new Messenger(WORKER_INDEX, this);
    this.projectsHandler = new ProjectsHandler(configs, this);
    this.puppeteerManager.once('puppeteer-start', () => {
      this.messenger.send({ event: 'worker-started' });
    });
    this.puppeteerManager.on('puppeteer-relaunched', () => {
      logger.debug(`Worker ${WORKER_INDEX} puppeteer-relaunched`);
    });
    this.puppeteerManager.on('puppeteer-browsercontext-closed', () => {
      logger.debug(`Worker ${WORKER_INDEX} puppeteer-browserContext-closed`);
    });
  }

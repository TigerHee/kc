/**
 * 获取 public-web 所有 gempool 币种详情路由
...
module.exports = async function() {
  logger.debug(`start get gempool detail routes for ${this.host}${this.entry}`);
  const api = `${this.apiHost}${REQUEST_API}`;

  const { THEME_DEFAULT } = this.configs.globalSupportThemes;
  const routesInfo = {
    routeSetName: 'gempool-detail',
    priority: 2,
    withoutLang: [],
    withLang: null,
    isApp: false,
  ...
  try {
    ...
    logger.error(`Request gempool detail ${api} error`, err);
    return routesInfo;
  }
};

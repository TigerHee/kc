/**
 * 获取 public-web 所有 盘前交易 币种路由 (app)
...
module.exports = async function() {
  logger.debug(`start get app-pre-market routes for ${this.host}${this.entry}`);
  const api = `${this.apiHost}${REQUEST_API}`;

  const { THEME_DEFAULT } = this.configs.globalSupportThemes;
  const routesInfo = {
    routeSetName: 'app-pre-market',
    priority: 3,
    withoutLang: [],
    withLang: null,
    isApp: true,
  ...
  try {
    ...
    logger.error(`Request app-pre-market ${api} error`, err);
    return routesInfo;
  }
};

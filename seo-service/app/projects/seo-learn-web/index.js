...
const genLearnRoutes = require('./gen-learn-routes');
const startServerlessJob = require('../common/serverless-gen');

class SeoLearnWebHandler extends BaseProjectHandler {
  constructor(configs, puppeteerManager, messenger) {
    super(configs, puppeteerManager, messenger);
    this.config = configs.projectConfigs['seo-learn-web'];
  ...
  async genRoutes(params) {
    ...
    for (const routeSet of params.routeSets) {
      ...
      switch (routeSet) {
        case 'learn':
          routesInfo = await genLearnRoutes.call(this, params.langs);
          break;
        default:
          break;
      }
...
module.exports = SeoLearnWebHandler;

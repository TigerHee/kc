...
async function visitUrl(
  ...
  await browserPage.evaluateOnNewDocument(() => {
    window.localStorage.setItem('kc_theme', 'dark');
  });
  // await browserPage.setExtraHTTPHeaders({ 'x-app-theme': 'dark' });
...
async function resolvePageContent(browserPage, projectConfig) {
  ...
  // 最后处理 useSSG 注入
  await resolveJSSStyles(browserPage, { isApp: true, theme: 'dark' });
}

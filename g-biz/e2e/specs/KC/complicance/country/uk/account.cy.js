...
describe(`【${url}】`, () => {
  ...
  it(`${url} 英国用户访问 account页面 合规巡检`, () => {
    cy.waitForSSG(url);

    cy.get('[data-inspector="account_overview_market_hot"]').should('not.exist');
    cy.get('[data-inspector="account_overview_get_started"]').should('not.exist');
    cy.get('[data-inspector="account_overview_vip_info"]').should('not.exist');
  });
});

...
describe(`【${url}】`, () => {
  ...
  it(`${url} 英国用户访问 主站header模块 合规巡检`, () => {
    cy.waitForSSG(url);

    cy.get('[data-inspector="inspector_header_url_/referral"]').should('not.exist');
    cy.get('[data-inspector="inspector_header_url_/land/KuRewards"]').should('not.exist');
  });

  it(`${url} 英国用户访问 footer主站模块 合规巡检`, () => {
    cy.waitForSSG(url);

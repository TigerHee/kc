...
import { Snackbar, useResponsive } from '@kux/mui';
...
import {
  DialogWrapper,
  TextDialogContentWrapper,
  TextRetainDialogContent,
  ImgWrap,
  DialogContent,
  TextTermTitle,
  TextTermExitButton,
...
const TextTermDialogContent = ({
  ...
  // 协议弹窗 勾选同意
  const onAgreeCheckInTerm = (termId) => {
    kcsensorsClick({
  ...
  // 协议弹窗 点击同意
  const onAgreeInTerm = (termId, idx) => {
    // 如果是从协议组件返回的回调，直接提交
        ...
        {!isLeave ? (
          <TextDialogContentWrapper>
            <TermContent
              ...
              extra={
                <TextTermExitButton variant="text" onClick={onLeaveInTerm}>
          ...
          <TextRetainDialogContent>
            <ImgWrap>
              <img
                alt="term info tip"
                src={getThemeImg({ light: DialogInfo, dark: DialogInfoDark })}
              />
            </ImgWrap>
            <DialogContent
...
// 正文式协议更新弹窗
export const TextTermDialog = ({ visible, ...props }) => {

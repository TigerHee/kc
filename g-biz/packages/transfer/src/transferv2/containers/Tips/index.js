...
import { useUnifiedStatus } from '../../hooks/unified';
import { useGetConfigByAccount } from '../../hooks/accounts';
import { StyledTip, StyledSavingsAlert, StyledAlert } from './style';
...
function MarginTip() {
  ...
  }, [recAccountType, recTag, dispatchTransfer]);
...
function UnifiedTip({ children }) {
  return <StyledAlert showIcon type="info" title={children} />;
}
...
export default function Tips() {
  ...
  const recAccountType = useFormField('recAccountType');
  const notAllowedAccounts = useTransferSelector((state) => state.notAllowedAccounts);
  const { unifiedSupport, isUnifiedMode, unifiedHasOpened } = useUnifiedStatus();
  ...
  // 统一账户相关提示
  if (unifiedSupport && isUnifiedMode) {
    if (recAccountType === 'TRADE') {
      return <UnifiedTip>{_t('9ab65b1543554000a319')}</UnifiedTip>;
    }
    if (recAccountType === 'CONTRACT') {
      return <UnifiedTip>{_t('07ca3d9fb87c4800ab71')}</UnifiedTip>;
    }
  }
  if (!isUnifiedMode && unifiedHasOpened && recAccountType === 'UNIFIED') {
    return <UnifiedTip>{_t('332b89e769424800af58')}</UnifiedTip>;
  }
  // 合约相关提示

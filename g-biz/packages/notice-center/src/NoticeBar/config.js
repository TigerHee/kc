...
const SvgIcon = styled.img`
  width: 16px;
  height: 16px;
  filter: drop-shadow(16px 0px ${(props) => props.theme.colors.primary});
 ...
 * @returns
 */
const assetRedirect = (url, { type, currency } = {}) => {
  const { noticePushTo: push } = window;
  const isCurrentPage = includes(
    window?.location?.pathname,
    type === 'deposit' ? '/assets/coin' : '/assets/withdraw',
  );
  if (!isCurrentPage) {
    push(url);
    return;
...
export const TYPE_MAP = (push) => {
  return {
    ...
    'notification.margin-enable-margin': {
      ...
      icon: (
        <IconWrapper>
          <SvgIcon src={marginInfoNoti} alt="" />
        </IconWrapper>
      ),
      // hookClick: (context) => {
      //   const { orderId } = context;
    ...
    'notification.margin-liquidation-alert': {
      ...
      icon: (
        <IconWrapper>
          <SvgIcon src={marginWarningNoti} alt="" />
        </IconWrapper>
      ),
      hookClick: () => {
    ...
    'notification.margin-liquidate-repay': {
      ...
      icon: (
        <IconWrapper>
          <SvgIcon src={marginSettleNoti} alt="" />
        </IconWrapper>
      ),
      hookClick: () => {
        push('/assets/margin-account');
    ...
    'notification.margin-negative-balance': {
      ...
      icon: (
        <IconWrapper>
          <SvgIcon src={marginDangerNoti} alt="" />
        </IconWrapper>
      ),
      hookClick: () => {
        push('/margin/borrow');
    ...
    'notification.target-lend-publish': {
      icon: (
        <IconWrapper>
          <SvgIcon src={bonusLoansNoti} alt="" />
        </IconWrapper>
      ),
      hookClick: () => {
    ...
    'notification.margin-bonus.bonus-send': {
      icon: (
        <IconWrapper>
          <SvgIcon src={marginBonusNoti} alt="" />
        </IconWrapper>
      ),
      hookClick: () => {
    ...
    'notification.margin-bonus.isolated-bonus-send': {
      icon: (
        <IconWrapper>
          <SvgIcon alt="" src={marginBonusNoti} />
        </IconWrapper>
      ),
      hookClick: () => {
        push('/assets/bonus/margin-bonus');
    ...
    'deposit.notice.bank.card.trade.success': {
      icon: (
        <IconWrapper>
          <SvgIcon alt="" src={marginWarningNoti} />
        </IconWrapper>
      ),
      hookClick: (context) => {
        const { orderId } = context;
...
// 法币充值渠道kyc通知点击时的事件
function handleDepositKycHookClick(context) {
  const { noticePushTo: push } = window;

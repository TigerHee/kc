...
export default extend(base, {
  namespace: 'notice_notice_center',
  state: {
  ...
  effects: {
    ...
    *feed({ payload: { type, message, extra, groupName, maxCount } }, { select, put }) {
      ...
      const { openingGroupMap } = yield select((state) => state.notice_notice_center);
      // 如果当前打开的notification数量达到最大值，关闭掉时间上最早进入的notification
      if (groupName && typeof groupName === 'string' && type.indexOf('notification.') === 0) {
        const { _publicMaxNoticeID } = yield select((state) => state.notice_notice_center);

...
const checkXGray = (baseConfig) => {
    const { sentry, projectName, environment, JsBridge } = baseConfig;
    const xGrayReloadKey = 'kucoinv2_xgray_reload';
    const debounceReload = debounce((reason = '') => {
        try {
            const x_gray_reload = Number(sessionStorage.getItem(xGrayReloadKey)) || 0;
            if (x_gray_reload <= 0) {
                return;
            }
            sentry?.captureEvent({
    ...
    return {
        ...
        async onFulfilled(response) {
            const isCanceled = response.headers['x-gray-canceled'] || response.headers['X-GRAY-CANCELED'];
            const canceledList = response.headers['x-gray-canceled-list'] || response.headers['X-GRAY-CANCELED-LIST'] || '';
            const isInCanceledList = canceledList.toLowerCase().indexOf(projectName) > -1;
            ...
            if (environment === RuntimeEnvironment.APP && !!JsBridge) {
                try {
                    ...
                    console.error(`[http-client] JsBridge updatePackageVersion异常,  ${e}`);
                }
            }
            return response;
        },

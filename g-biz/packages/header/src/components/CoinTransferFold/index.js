...
import { CURRENCY_CHARS } from '../../../../transfer/src/utils/currencyMap';
import { numberFixed } from '../../common/tools';

// 获取当前语言环境的小数分隔符
const getDecimalSeparator = (locales) => {
  return (1.1).toLocaleString(locales).substring(1, 2);
};

const formatPrice = (price, locales, currency = '') => {
  if (price > 1) {
    ...
    if (decimalPart) {
      ...
      return formattedDecimal
        ? `${currency}${formatter.format(integerPart)}${getDecimalSeparator(
            locales,
          )}${formattedDecimal}`
  ...
  if (price < 1) {
    ...
    // 有效数字部分，保留 5 位有效数字
    const effectiveDigits = decimalPart
      .slice(leadingZerosCount, leadingZerosCount + 5)
      .replace(/0+$/, '');
...
const PriceFormatter = ({ price, locales, currency = '' }) => {
  ...
  return (
    <span>
      {base}
      {subscript !== undefined && (
        <span style={{ verticalAlign: 'sub', fontSize: '0.75em' }}>{subscript}</span>
      )}
      {digits}
    </span>
  );
...
const CoinTransferFold = ({
  ...
  // 对于用户选择的法币和汇率从子应用读取，子应用不存在在则默认美元，汇率为1，后期可优化不存在请求数据
  const currency =
    useSelector((state) => state?.currency?.currency) || window._DEFAULT_RATE_CURRENCY_ || 'USD'; // 当前用户选择的法币单位
  const prices = useSelector((state) => state?.currency?.prices); // 转换比例
  const rate = prices?.[quoteCurrency] || '1'; // 转换比例
  // 检查是否有效、0 展示--
  ...
  const target = multiply(rate, value, 40);
  const _lang = currentLang.replace('_', '-');
  return <PriceFormatter price={target} locales={_lang} currency={legalChars} />;
};

...
const defaultConfig = {
  ...
  // 合约交易地址 交易大厅版本
  KUMEX_TRADE: `${window.location.origin}/trade/futures`,
  ...
  botGlobalListShow: () => window._BRAND_SITE_ === 'KC', // true 展示主站列表, false展示多站点列表
  // 机器人菜单配置
...
const KC = {
  ...
  KUMEX_TRADE: futuresNewEntryEnabled
    ? `${window.location.origin}/trade/futures`
    : `${window.location.origin}/futures/trade`,
...
const TH = {
  ...
  filterCurrency: (currencies) =>
    currencies.filter((currency) => currency === 'THB' || currency === 'USD'), // 过滤货币

...
import CoinCodeToName from './common/CoinCodeToName';
import CoinSelectDialog from './common/CoinSelectDialog';
...
const CurrencySelect = ({
  ...
  tradeDirection,
  updateAtOpen = true,
  commonlyCacheKey = 'convert_commonly_list',
  readOnly = false,
  ...otherProps
}) => {
  ...
  const loading = useSelector((state) => state.loading.effects[`${NAMESPACE}/queryCurrencyList`]);
  ...
  useEffect(() => {
    if (open && updateAtOpen) {
      dispatch({
        type: `${NAMESPACE}/queryCurrencyList`,
  ...
  return (
    <>
      ...
      <CoinSelectDialog
        ...
        commonlyCacheKey={commonlyCacheKey}
        {...(isLogin ? { amountKey: 'availableBalance' } : null)}
        extraList={[
          {
            dataSource: hots,
            title: _t('dGXZ1xSsxk9wp7Fo6YeUDp'),
          },
        ]}
      />
    </>

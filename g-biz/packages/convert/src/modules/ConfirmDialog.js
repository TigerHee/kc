...
import PriceNotice from './MarketForm/PriceNotice';
import ButtonWrapper from './MarketForm/ButtonWrapper';
import { NAMESPACE, ORDER_TYPE_MAP, ACCOUNT_TYPE_LIST_MAP } from '../config';
import { getInnerUrl } from '../utils/tools';
import { preloadTaxInfoCollectDialog } from './TaxInfoCollectDialog';
import MarkertSubmitButton from './MarketForm/MarkertSubmitButton';
...
const ConfirmDialog = ({
  ...
  const { limitOrderValidityPeriodDay } = baseConvertConfig || {};
  const isBuy = inversePriceSymbol === _priceSymbol;

  useEffect(() => {
    if (open) {
      preloadTaxInfoCollectDialog();
      kcsensorsManualTrack({ spm: ['convertForm', isMarketOrder ? '1' : '2'] }, 'expose');
    }
  }, [open, isMarketOrder]);
  ...
  const renderButtons = () => {
    if (isMarketOrder) {
      return <MarkertSubmitButton loading={confirmLoading} open={open} onClick={onOk} />;
    }
    if (showPriceDanger) {
      return (
        <>
    ...
    return (
      <Button
        ...
        onClick={onOk}
        data-inspector="convert_limit_confirm_button"
      >
  ...
  return (
    ...
    >
      <Banel>
        <CurrencyInfo>
          <CoinIcon size={28} coin={fromCurrency} />
          <CoinName>
      ...
      <Content>
        {!isMarketOrder && (
          <Row>
            <FlexBox>
              {_t('nmDQYPhTvaprEUDn6eDzac')}{' '}
              <Tooltip title={_t('kEp69coZeZ44vPQzUrSr1P')}>
                <TooltipContent>
                  <ICQuestionOutlined size={14} />
                </TooltipContent>
              </Tooltip>
            </FlexBox>
            <Value>{_t('7RnQHRfeJVJMaEwuJskFW1', { n: limitOrderValidityPeriodDay || '-' })}</Value>
          </Row>
        )}
        <Row>
          <FlexBox>
        ...
        <Row>
          {_t('6dbLkJiVyhUxcLrEyuuLbV')}
          <Value>{ACCOUNT_TYPE_LIST_MAP[accountType].label()}</Value>
        </Row>

...
import { NAMESPACE, MAX_PRICE_EXPIRE_TIME, ORDER_TYPE_ENUM, ORDER_TYPE_MAP } from '../../config';
...
const MarketForm = ({ visible, onCancel }) => {
  const [form] = useForm();
  ...
  const { run, cancel, onlyCancel, isRunning } = usePolling(
    `${NAMESPACE}/${ORDER_TYPE_MAP[ORDER_TYPE_ENUM.MARKET].queryPriceEffectName}`,
    `${NAMESPACE}/${ORDER_TYPE_MAP[ORDER_TYPE_ENUM.MARKET].triggerPollingEffectName}`,
  );
  ...
  useEffect(() => {
    ...
    kcsensorsManualTrack({ spm: ['convertForm', '1'] }, 'expose');
  }, [dispatch]);
  ...
  const handleSubmit = useEventCallback(() => {
    ...
    if (!priceInfo?.tickerId || Date.now() - priceInfo.updateAt > MAX_PRICE_EXPIRE_TIME) {
      sentryCaptureEvent({
        ...
        message: `market convert query price error: ${
    ...
    dispatch({
      type: `${NAMESPACE}/${ORDER_TYPE_MAP[ORDER_TYPE_ENUM.MARKET].confirmOrderEffectName}`,
      payload: {
        accountType,
        channel: 'WEB_CONVERT',
  ...
  const handleOpenConfirmModal = useEventCallback(() => {
    ...
    // 手动重置轮训，方便按钮倒计时
    queryPrice();
    kcsensorsManualTrack({ spm: ['market', 'placeOrder'] }, 'page_click');
  });
  ...
  return (
    <StyledForm form={form} onFinish={handleOpenConfirmModal} data-inspector="convert_market_form">
      <FieldWrapper {...fromField.wrapperProps}>
        <FormItem {...fromField.formItemProps}>
          <InputNumber
            {...fromField.inputNumberProps}
            data-inspector="convert_market_form_from_input"
          />
        </FormItem>
      </FieldWrapper>
      ...
      <FieldWrapper {...toField.wrapperProps}>
        <FormItem {...toField.formItemProps}>
          <InputNumber
            {...toField.inputNumberProps}
            data-inspector="convert_market_form_to_input"
          />
        </FormItem>
      </FieldWrapper>
      <EtfAlert />
      ...
      {Boolean(priceInfo?.tickerId) && (
        <>
          <ConfirmDialog
            data-inspector="convert_market_confirm_dialog"
            open={open}
            onOk={handleSubmit}
            priceInfo={priceInfo}
          ...
          />
          <ResultDialog
            data-inspector={`convert_market_result_${result?.type}_dialog`}
            {...result}
            pageType="history"
            onCancel={setResult}

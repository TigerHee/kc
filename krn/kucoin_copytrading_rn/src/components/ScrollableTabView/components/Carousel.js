...
export class Carousel extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      this.firstMountTime = new Date().getTime();
    }, 1);
  }
    ...
    const hasNewSize =
      ...
      (!this.props.vertical &&
        ...
        (prevProps.itemWidth !== this.props.itemWidth ||
          prevProps.sliderWidth !== this.props.sliderWidth)); // Prevent issues with dynamically removed items
  ...
  _needsScrollView() {
    const {useScrollView} = this.props; // Android's cell renderer is buggy and has a stange overflow
  ...
  _getWrappedRef() {
    ...
    return (
      this._carouselRef && // @ts-expect-error This is for before 0.62
  ...
  _onTouchStart(event) {
    const {onTouchStart} = this.props; // `onTouchStart` is fired even when `scrollEnabled` is set to `false`
  ...
  _onScroll(event) {
    ...
    if (nextActiveItem !== this._onScrollActiveItem) {
      ...
      // hack 代码 优化 安卓 RTL 场景 scrollview 长列表自动滚动问题
      // https://github.com/facebook/react-native/pull/37651
      // https://github.com/facebook/react-native/issues/26436
      const hackLimitOptimizeFlag =
        IS_RTL &&
        IS_ANDROID &&
        this.firstMountTime &&
        new Date().getTime() - this.firstMountTime < 500;
      !hackLimitOptimizeFlag &&
  ...
  _onLayout(event) {
    const {onLayout} = this.props; // Prevent unneeded actions during the first 'onLayout' (triggered on init)
  ...
  _renderItem({item, index}) {
    ...
    const mainDimension = this.props.vertical
      ? {height: this.props.itemHeight || 1000}
      : {width: this.props.itemWidth || 1000};
  ...
  render() {
    ...
    const isCustomScrollView = this._needsScrollView() || !Animated.FlatList;

    return isCustomScrollView ? (

...
const isAndroidRTL = isRTL && isAndroid;
...
export default class ScrollableTabView extends PureComponent {
  ...
  _initialProperty() {
    ...
    this._throttleCallback = _throttle(
      this._onTabviewChange.bind(this, true),
      screenScrollThrottle,
      {
        leading: false,
        trailing: true,
      },
  ...
  _initial(props = this.props, isProcess = false) {
    ...
    if (props.firstIndex > Math.max(this.stacks.length - 1, 0))
      this._displayConsole(
        'firstIndex cannot exceed the total number of stacks.length',
        CONSOLE_LEVEL.ERROR,
      );
  }
  ...
  _toProcess(props) {
    if (
      props.stacks &&
      props.stacks.length &&
      props.stacks.length != this.stacks.length &&
      props.firstIndex != this.state.checkedIndex
    ) {
  ...
  _makeStacksID(item) {
    if (item && !item.__id__)
      item.__id__ = `${Math.random().toString().slice(2, 8)}_${Date.now()
        .toString()
        .slice(2, 8)}`;
  }
  ...
  _getWrapChildren(props) {
    const stacks =
      ...
      props.stacks.map((item, index) => {
        if (item.screen) {
          if (this.isClassComponent(item.screen) && !item.screen.__HOCNAME__) {
            this._makeStacksID(item);
            item.screen = HocComponent(
              item.screen,
              this._setCurrentRef(index, item.__id__),
            );
          } else {
            this._makeStacksID(item);
  ...
  _setCurrentRef(index, id) {
    return ref => {
      if (this.state.refsObj[index] && this.state.refsObj[index] === ref)
        return;
  ...
  _getFirstIndex() {
    ...
    const getIndex = (array, firstIndex, isRTL) => {
      const arrayLength = array.length;

      if (isRTL) {
        // 从后面开始数的 index 值
        return arrayLength - 1 - firstIndex;
      } else {
        // 返回 firstIndex
        return firstIndex;
      }
    ...
    if (typeof firstIndex === 'number' && stacks && stacks.length) {
      ...
      // return getIndex(stacks, this.props.firstIndex, isRTL);
    } else {
      return null;
    }
  }
  ...
  _renderSticky() {
    ...
    if (
      stacks &&
      stacks.sticky &&
      typeof stacks.sticky === 'function' &&
      ref &&
      stacks.__id__ === ref.__id__
    ) {
      // 用于自动同步 Screen 数据流改变后仅会 render 自身 Screen 的问题，用于自动同步 screenContext 给吸顶组件
      if (
        this.props.syncToSticky &&
        !ref.__isOverride__ &&
        this.isClassComponent(ref.constructor)
      ) {
      ...
      return (
        <stacks.sticky
          {...this._getProps(this.props.mappingProps || {})}
          screenContext={ref}
        />
      );
    }
  ...
  _measureTab(pageIndex, {nativeEvent}) {
    ...
    this.tabsMeasurements[pageIndex] = {
      left: x,
      right: x + width,
      width,
      height,
  ...
  _renderTab({item, index}) {
    ...
    const _tabWrapStyle =
      typeof tabWrapStyle === 'function'
        ? tabWrapStyle(item, index, _checked)
        : tabWrapStyle;
    const _tab =
      typeof item.tabLabelRender === 'function'
        ? item.tabLabelRender(item.tabLabel, index, _checked)
        : item.tabLabel;
    return (
      <View
        ...
        <TouchableOpacity
          ...
          <View style={tabInnerStyle}>
            ...
            {!hiddenUnderLine && !tabsEnableAnimated && _checked && (
              <View style={_tabUnderlineStyle} />
            )}
          </View>
        </TouchableOpacity>
      </View>
  ...
  _getTabUnderlineInterpolateArgs(tabsEnableAnimatedUnderlineWidth) {
    ...
    if (
      maxTranslateXCount === this.tabUnderlineInterpolateArgs.inputRange.length
    )
      return this.tabUnderlineInterpolateArgs;
    const _outputRange = [];
    const _inputRange = Array.from({length: maxTranslateXCount}, (v, k) => {
      _outputRange.push(
        k % 2 ? this.tabWidth / tabsEnableAnimatedUnderlineWidth : 1,
      );
  ...
  _renderAnimatedTabUnderline() {
    ...
    if (!_tabUnderlineStyle.top && _tabUnderlineStyle.height)
      _tabUnderlineStyle.top =
        this.layoutHeight.tabs - _tabUnderlineStyle.height;
    ...
    if (tabsEnableAnimatedUnderlineWidth) {
      if (tabsEnableAnimatedUnderlineWidth >= this.tabWidth / 2)
        this._displayConsole(
          'The value of tabsEnableAnimatedUnderlineWidth we recommend to be one-third of tabStyle.width or a fixed px',
        );
      interpolateAnimated.marginLeft =
        this.tabWidth / 2 - tabsEnableAnimatedUnderlineWidth / 2;
      ...
      interpolateAnimated.transform.push({
        scaleX: this.scroll2HorizontalPos.interpolate(
          this._getTabUnderlineInterpolateArgs(
            tabsEnableAnimatedUnderlineWidth,
          ),
        ),
      });
    ...
    if (errorProps[propName] && property) {
      errorProps[propName].forEach(errorProperty => {
        if (errorProperty in property) {
          this._displayConsole(
            `Prop ${propName} is not allowed to configure the ${errorProperty} property`,
            level,
          );
        }
      });
  ...
  _renderTabs() {
    ...
    const renderTab =
      !(oneTabHidden && this.tabs && this.tabs.length === 1) && tabsShown;
    const _tabsStyle = Object.assign({}, styles.tabsStyle, tabsStyle);

    this.layoutHeight.tabs = renderTab ? _tabsStyle.height : 0;
    this.tabWidth = width;
    const renderTabList = this.tabs;
    return (
      renderTab &&
      this.tabs?.length && (
        <View style={tabsOuterWrapStyle}>
          <HorizontalScrollContainer style={[_tabsStyle]} bounces={false}>
            {renderTabList.map((tab, index) =>
              this._renderTab({item: tab, index}),
            )}
            {tabsEnableAnimated &&
              this.state.checkedIndex !== null &&
              this._renderAnimatedTabUnderline()}
          </HorizontalScrollContainer>
          {tabRightNode ? tabRightNode : null}
        </View>
      )
    );
  }
  ...
  _resetOtherRefs() {
    ...
    if (this.state.refsObj && this.state.refsObj[checkedIndex])
      this.state.refsObj[checkedIndex] = null;
  ...
  _onTabviewChange(isCarouselScroll, index) {
    ...
    const {enableCachePage, toHeaderOnTab, toTabsOnTab, onTabviewChanged} =
      this.props;
    if (index === this.state.checkedIndex) {
      if (!isCarouselScroll && toHeaderOnTab)
        return this._scrollTo(
          -(this.layoutHeight.header + this.layoutHeight.stickyHeader),
        );
    ...
    if (!enableCachePage) {
      ...
      if (!this._getLazyIndexs(index)) {
        state.lazyIndexs.push(index);
        isFirst = true;
      }
    }
    this.setState(state, () => {
  ...
  _getScreenHeight() {
    this.layoutHeight.screen =
      this.layoutHeight.container -
      (this.layoutHeight.header +
        this.layoutHeight.stickyHeader +
        this.layoutHeight.tabs);
      ...
      (!this.props.lazy ? true : this.state.checkedIndex === index) && (
        <View
          ...
          <item.screen
            {...this._getProps(
              mappingProps,
              !this.isClassComponent(item.screen) && item.screen,
            )}
            {...(item.toProps || {})}
          />
        </View>
  ...
  _onEndReached() {
    const next = () => {
      ...
      !!ref && this.isClassComponent(ref.constructor)
        ? ref &&
          ref.onEndReached &&
          typeof ref.onEndReached === 'function' &&
          ref.onEndReached()
        : triggerEndReached(ref);
    };
    if (this.state.checkedIndex != null) {
      const {onBeforeEndReached} = this.props;
      onBeforeEndReached && typeof onBeforeEndReached === 'function'
  ...
  _renderTitle = () => {
    ...
    return (
      <Animated.View
        style={[
          {
            height: this.scroll2VerticalPos.interpolate(
              Object.assign(
                this.titleInterpolateArgs.height,
                interpolateHeight,
              ),
            ),
            opacity: this.scroll2VerticalPos.interpolate(
              Object.assign(
                this.titleInterpolateArgs.opacity,
                interpolateOpacity,
              ),
            ),
            overflow: 'hidden',
  ...
  _setScrollHandler2Vertical() {
    ...
    const argMapping = [];
    argMapping.push({
      nativeEvent: {contentOffset: {y: this.scroll2VerticalPos}},
    });
    this._onScrollHandler2Vertical = Animated.event(
      argMapping,
      scrollEventConfig,
    );
  }
  ...
  _onScroll2Horizontal(event) {
    ...
    if (typeof onScroll2Horizontal === 'function' && event)
      onScroll2Horizontal(event);
  }
  ...
  _setScrollHandler2Horizontal() {
    ...
    const argMapping = [];
    argMapping.push({
      nativeEvent: {contentOffset: {x: this.scroll2HorizontalPos}},
    });
    this._onScrollHandler2Horizontal = Animated.event(
      argMapping,
      scrollEventConfig,
    );
  }
  ...
  render() {
    ...
    // 解决安卓 RTL 时 tabs 已反转，Panel 未反转问题
    const renderStacks = isAndroidRTL
      ? this.stacks.slice().reverse()
      : this.stacks;
    return (
      <View
        onLayout={({nativeEvent}) => {
...
const styles = StyleSheet.create({
  ...
  tabStyle: {},
  textStyle: {},
  tabUnderlineStyle: {height: 2, borderRadius: 2},
});

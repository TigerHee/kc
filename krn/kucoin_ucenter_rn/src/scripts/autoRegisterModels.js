...
function register() {
  ...
  data.forEach(item => {
    if (/.*.js$/.test(item) && item !== '_index.js') {
      ...
      keys.push(key);
      outputScript += `export ${key} from './${item}';\n`;
    }
  });
  if (keys.length) {
...
function listenDir() {
  fs.watch(modelsPath, (event, filename) => {
    const changeKey = getKeyByFileName(filename);
    if (!keys.includes(changeKey) && changeKey !== '_index') register();
  });
}

register();
if (!NO_WATCH) listenDir();

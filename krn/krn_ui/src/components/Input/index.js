...
const InputWrapper = styled.View`
  ...
  border-radius: 8px;
  padding: 0 16px;
  min-height: ${({ size }) => (size === 'large' ? '56px' : size === 'small' ? '40px' : '48px')};
  flex-direction: row;
  align-items: center;
`;
...
const Input = ({
  ...
  return (
    <InputContainer style={styles.inputContainer}>
      <InputWrapper style={styles.inputWrapper} colorType={colorType} size={size}>
        {label ? (
          <InputLabelView style={styles.inputLabelView}>
            <InputLabelText colorType={colorType} disabled={disabled} style={styles.inputLabelText}>
              {label}
            </InputLabelText>
          </InputLabelView>
        ) : null}
        ...
        {allowClear && !!value ? (
          ...
          >
            <ClearButton size={size} style={styles.clearButton} source={input_clear_icon} />
          </ClearButtonView>
        ) : null}

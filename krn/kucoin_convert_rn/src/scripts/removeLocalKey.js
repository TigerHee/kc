 ...
 * 4. 去掉 sourceLocalesPath 的值，设置 targetDiffPath 的值，执行: node scripts/removeLocalKey.js
...
// 目标项目locales目录
const targetLocalesPath = path.join(__dirname, '../locales');

// 目标项目diff路径
const targetDiffPath = path.join(__dirname, '../../.neeko/diff.json');

// 是 gbiz 或者 main-web,public-web
// const isGbiz = false;

const jsTemplate = 'export default &{data}';

const parseJsTemplate = (data, temp) => {
  // let result = temp.replace(/export default /, () => code);

  let result = temp.replace(/&\{data\}/, () => JSON.stringify(data, null, 2));
...
const init = () => {
  ...
  try {
    fs.readdirSync(targetLocalesPath).forEach(file => {
      var targetFilePath = path.join(targetLocalesPath, file);
      console.log('targetFilePath === ', targetFilePath);

      var stat = fs.statSync(targetFilePath);
      // console.log('stat === ', stat);
      if (stat.isFile() && path.extname(targetFilePath) === '.js') {
        var code = fs.readFileSync(targetFilePath, 'utf8');
        const targetContent = JSON.parse(code.replace('export default ', ''));

        let fullContent = {...targetContent};

        // 有源项目locales目录就去读，没有就用目标项目的数据处理
        // if (sourceLocalesPath) {
        //   var sourceFilePath = path.join(sourceLocalesPath, file);
        //   var sourceCode = fs.readFileSync(sourceFilePath, 'utf8');

        //   if (isGbiz) {
        //     const fn = new Function(
        //       sourceCode.replace('export default', 'return'),
        //     );
        ...
        fs.writeFileSync(targetFilePath, parseJsTemplate(content, jsTemplate));
      }
    });
  } catch (error) {
    console.error(error);

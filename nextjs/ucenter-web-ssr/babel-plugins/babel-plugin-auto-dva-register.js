...
module.exports = function (babel) {
  const t = babel.types;
  // console.log('[auto-dva-register] Babel 插件加载');
  return {
    ...
    visitor: {
      Program(pathNode, state) {
        ...
        const importRegister = t.importDeclaration(
          ...
          t.stringLiteral('@/tools/dva')
        );

        // 插入位置：在已有 import 之后
        let insertIndex = 0;
        while (insertIndex < pathNode.node.body.length && t.isImportDeclaration(pathNode.node.body[insertIndex])) {
          insertIndex++;
        }
        ...
        const initRegSet = t.variableDeclaration('const', [
          t.variableDeclarator(
            ...
            t.logicalExpression(
              ...
              t.assignmentExpression('=', t.memberExpression(t.identifier('globalThis'), regSetId), t.newExpression(t.identifier('Set'), []))
            )
          ),
        ]);
        ...
        for (const ns of Array.from(namespaces)) {
          ...
          const candidates = modelFiles.filter((p) => {
            ...
            return lower === `@/__models/${ns.toLowerCase()}` || lower.endsWith(`/${ns.toLowerCase()}`);
          });
          ...
          if (chosen) {
            ...
            const func = t.arrowFunctionExpression(
              [],
              t.blockStatement([t.returnStatement(t.awaitExpression(t.callExpression(t.import(), [t.stringLiteral(chosen)])))]),
              true
            );
            ...
            const func = t.arrowFunctionExpression(
              ...
              t.blockStatement([
                t.throwStatement(
                  t.newExpression(t.identifier('Error'), [t.stringLiteral(`[auto-dva-register] 未找到对应 model: ${ns}`)])
                ),
              ]),
              true
            );
        ...
        const loadersDecl = t.variableDeclaration('const', [
          t.variableDeclarator(t.identifier('__DVA_MODEL_LOADERS'), t.objectExpression(loaderProps)),
        ]);
        ...
        // ===============================
        pathNode.node.body.splice(insertIndex, 0, importRegister, initRegSet, loadersDecl, ...iifeAST);
      },
    },
  };
};

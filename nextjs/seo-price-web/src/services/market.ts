 ...
 */
import { postJson, post, pull as originPull } from 'gbiz-next/request';
 ...
 */
export async function getCoinsCategory(status?: number) {
 ...
 * 获取交易所所有启用交易对
 */
export async function getMarketSymbols({ base, market }: { base?: string; market?: string; }) {
...
export async function getCollectionContracts() {
  const { KUMEX_GATE_WAY } = getSiteConfig();

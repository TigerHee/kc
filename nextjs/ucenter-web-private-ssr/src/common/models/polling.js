 ...
 */
import { genCompare } from 'common/models/utils/modelHelper';
import { isFunction, isFinite, isPlainObject } from 'lodash-es';
import { delay as delaySaga } from 'utils/delay';
      ...
      const runTick = function* runTick() {
        try {
          while (polling) {
            ...
            if (isFunction(optInterval)) {
              optInterval = yield call(optInterval, effectPayload);
            }
            if (!isFinite(optInterval)) {

...
class MyDocument extends Document {
  ...
  render() {
    ...
    const { _DEFAULT_LOCALE_ } = bootConfig;
    const isProd = process.env.NODE_ENV === 'production';
    const appName = process.env.NEXT_PUBLIC_APP_NAME;
    const brandSite = process.env.NEXT_PUBLIC_BRAND_SITE;

    // 仅在生产环境下拼接前缀
    const patchScript = isProd
      ? `
        (function() {
          const origFetch = window.fetch;
          window.fetch = function(input, init) {
            input = input.replace('/default/', '/${_DEFAULT_LOCALE_ || 'en'}/');
            if (typeof input === 'string' && input.startsWith('/_next/data/')) {
              input = '/ssr/${brandSite}/${appName}' + input;
            }
    ...
    return (
      ...
      >
        <Head>
          ...
          <link rel="preconnect" href={'https://bigdata-scfx-push.kucoin.plus'} />
          <link
            rel="preload"
          ...
          />
          <noscript>
            <link rel="stylesheet" href={`${cdnPrefix}/natasha/npm/@kux/font/css.css`} />
            <link
              rel="stylesheet"

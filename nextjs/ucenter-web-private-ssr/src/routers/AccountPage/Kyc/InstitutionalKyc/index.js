...
const InstitutionalKycWithLayout = (props) => {
  return (
    <ErrorBoundary scene={SCENE_MAP.kyc.kyc_institutional_kyc}>
      <AccountLayout>
        <InstitutionalKyc {...props} />
      </AccountLayout>
    </ErrorBoundary>
  );
};
...
export default connect(({ kyc, user: { user: { isSub = false } = {} } = {} }) => ({
  ...kyc,
  isSub,
}))(injectLocale(InstitutionalKycWithLayout));

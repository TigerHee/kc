...
export default extend(base, {
  ...
  effects: {
    *authCodeJump({ payload }) {
      let host = '/';
      const isDev = process.env.NODE_ENV === 'development' || typeof window !== 'undefined' ? (window.location.host.includes('localhost') || window.location.host.includes('127.0.0.1')) : false;
      if (!isDev) {

...
import { AccountBox, OptionLable, SymbolLabel } from './styles/index/style.js';
...
export const ACCOUNT = [
  ...
  // },
  {
    ...
    color: '#F8B200',
    label: () => _t('ecdf35ac6b904000a464'),
    labelComp: (lang) => (
      <AccountNameWithIcon
        bc="#F8B200"
        label={_t('ecdf35ac6b904000a464')}
        iconText={lang === 'zh_CN' ? '期' : 'Op'}
        icon={<SvgIcon width="18px" height="18px" iconId="Options" />}
      />
    ),
    supportDirection: ['from', 'to'], // 支持的方向
    // 支持划入该账户的账户
    ...
    checkCurrencyIsSupport: ({ currency }) => {
      return currency === bootConfig._BASE_CURRENCY_;
    },
  },
];
const ACCOUNT_MAP = {};
...
export const SUB_ACCOUNT = [
  ...
  },
  {
    ...
    label: (lang) => (
      <AccountNameWithIcon
        bc="#F8B200"
        label={() => SubAccountNameLabel(_t('master.account'), _t('ecdf35ac6b904000a464'))}
        iconText={lang === 'zh_CN' ? '期' : 'Op'}
        icon={<SvgIcon width="18px" height="18px" iconId="Options" />}
      />
    ),
  ...
  {
    // 子账号-期权账号
    key: `SUB_${ACCOUNT_CODE.OPTION}`,
    color: '#F8B200',
    label: (lang) => (
      <AccountNameWithIcon
        bc="#F8B200"
        label={() => SubAccountNameLabel(_t('subaccount.subaccount'), _t('ecdf35ac6b904000a464'))}
        iconText={lang === 'zh_CN' ? '期' : 'Op'}
        icon={<SvgIcon width="18px" height="18px" iconId="Options" />}
      />
    ),
...
// 接口权限名称到多租户配置系统权限名称的映射
export const typesKeyMap = {
  TRADE: 'spot',
  MARGIN: 'margin',
  CONTRACT: 'futures',
  OPTION: 'option',
  ISOLATED: 'margin', // 逐仓&全仓杠杠账户都是margin
};

// 接口权限名称到多租户配置系统权限名称的映射
export const subTypesKeyMap = {
  SUB_TRADE: 'spot',
  SUB_MARGIN: 'margin',
  SUB_CONTRACT: 'futures',
  SUB_OPTION: 'option',
  SUB_ISOLATED: 'margin', // 逐仓&全仓杠杠账户都是margin
};

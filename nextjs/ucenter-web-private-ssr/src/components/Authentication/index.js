...
import { Box, Button, Form, Spin, useSnackbar } from '@kux/mui';
...
const FACE_FAIL_CODES = {
  2022501: _t('authentication.faceVerify.fail1'),
  2022503: _t('authentication.faceVerify.fail2'),
  2022505: _t('authentication.faceVerify.fail3'),
  200: _t('authentication.faceVerify.fail3'),
};
...
export default (props) => {
  ...
  const isH5 = !rv?.sm;
  const { namespace, onSubmit, nextStep, uploadParams } = props;
  const isPhone = namespace === 'rebind_phone';
  const { configs = {}, kycCode, isKYC2 } = useSelector((state) => state[namespace]);
  ...
  const loading = useSelector((state) => {
    return (
      state.loading.effects[`${namespace}/checkFacePic`] ||
      state.loading.effects[`${namespace}/pullKycInfo`]
    );
  });

  const loadingSubmit = useSelector((state) => {
    return (
      state.loading.effects['reset_g2fa/submitAuthentication'] ||
      state.loading.effects['rebind_phone/submitNewPhone']
    );
  });
  ...
  // ios 3.117 以前的版本baidu人脸有问题，直接走手持
  ...
  const handleSubmit = () => {
    form
      ...
      .then(() => {
        ...
        isPhone ? nextStep() : onSubmit();
      })
      .catch((err) => {
        console.log(err);
        message.error(_t('authentication.upload.error'));
      });
  };
  ...
  // 点击身份验证页面按钮，下一步or重试or提交
  const handleResultClick = () => {
    // 验证成功：谷歌不可用直接提交 或者 手机不可用不提交进入下一步，进入绑定新手机后再提交
  ...
  if (isKYC2 && isInApp && retrySDKTimes < MAX_SDK_TIMES) {
    return (
      <Kyc2InApp>
        ...
        <Button size="large" onClick={handleToScan} fullWidth={true} loading={loadingSDK}>
  ...
  return (
    <IdentityForm form={form}>
      {props?.prompt && <div style={{ marginBottom: '40px' }}>{props?.prompt}</div>}
      {needFrontBackPic && (
        <>
          <TitleWrapper>
          ...
          <UploadItem id="frontPic" namespace={namespace} uploadParams={uploadParams} />
          <Box style={{ height: '4px' }} />
          <FormItemTitle>{_t('kyc.form.backPhoto')}</FormItemTitle>
          <UploadItem id="backPic" namespace={namespace} uploadParams={uploadParams} />
          <Box style={{ height: '4px' }} />
      ...
      {needHandPic && (
        <>
          ...
          <TitleWrapper>
            <NeededItemsPrefix0>*</NeededItemsPrefix0>
            <Title style={{ fontSize: isH5 ? '18px' : '20px' }}>
              {_t('selfService.auth.guide2')}
            </Title>
          </TitleWrapper>
          <Box style={{ height: '20px' }} />
          <FormItemTitle>{_t('selfService.auth.guide2')}</FormItemTitle>
          <UploadItem id="handPic" namespace={namespace} uploadParams={uploadParams} />
      ...
      <FormItem>
        <Button
          data-testid="authentication-submit"
          size="large"

...
import { Alert, Button, Spin, styled, useSnackbar } from '@kux/mui';
...
const ScanQRCode = ({ advanceUrl, expireSecond, onOk, onSupplierCallback }) => {
  ...
  const language = currentLang === bootConfig._DEFAULT_LANG_ ? '' : `/${alllanguage[currentLang]}`;
    ...
    <Wrapper>
      ...
      <QRCodeWrapper>
        <Spin spinning={loading} size="xsmall">
          <QRCodeSVG value={url} size={isH5 ? 120 : 212} level="Q" />
          {isExpired ? (
            <Mask onClick={getUrl}>

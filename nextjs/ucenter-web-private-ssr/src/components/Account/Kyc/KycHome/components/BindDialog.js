...
export default ({ open, onCancel }) => {
  const { emailValidate } = useSelector((state) => state.user?.user ?? {});
  ...
  return (
    <ExDialog
      ...
      onOk={() => {
        push(emailValidate ? '/account/security/phone' : '/account/security/email');
      }}
      centeredFooterButton
      showCloseX={false}
    >
      <PreAuthBox>
        <PreAuthIcon src={emailValidate ? phoneIcon : emailIcon} />
        <PreAuthTitle>
          {emailValidate ? _t('23c84fadda274000ae0f') : _t('0c217e4a4ace4000a800')}
        </PreAuthTitle>

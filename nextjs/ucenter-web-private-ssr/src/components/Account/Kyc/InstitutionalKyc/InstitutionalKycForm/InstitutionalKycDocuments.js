...
import { forEach, forIn, isArray, isEmpty, map, filter } from 'lodash-es';
...
const InstitutionalKycDocuments = ({
  ...
  const handlerResultSubmit = () => {
    // 将数据的key和value对应
    let obj = {};
    forIn(MODEL_TO_FIELD, (value, key) => {
      let tempArr = DATA_MODEL[key];
      if (isArray(tempArr) && !isEmpty(tempArr)) {
        tempArr = map(tempArr, (item) => item.key);
      }
      obj[value] = tempArr;
    });
    return obj;
  ...
  const validateRuleFlag =
    ...
    (isArray(companyDetail.authorizeAttachments) &&
      isEmpty(companyDetail.authorizeAttachments));

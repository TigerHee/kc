...
import { Menu, MenuItem, styled } from '@kux/mui';
...
import { trackClick } from '@/tools/ga';
...
const AccountMenu = () => {
  ...
  // 如果不是服务端渲染，并且有 multiSiteConfig, 才按照客户端的逻辑渲染
  // 否则都展示
  const init = !IS_SERVER_ENV && multiSiteConfig;
  ...
  const menus = useMemo(() => {
    ...
    if (!init || pageDirectorys?.includes('overview')) {
      _menus.push({
    ...
    if (!init || pageDirectorys?.includes('accountsecurity')) {
      _menus.push({
        title: (
    ...
    if ((!init || pageDirectorys?.includes('kyc')) && !isSub) {
      _menus.push({
    ...
    if ((!init || pageDirectorys?.includes('api')) && !isSub) {
      _menus.push({
    ...
    if ((!init || pageDirectorys?.includes('subuser')) && !isSub) {
      _menus.push({
    ...
    if (!init || pageDirectorys?.includes('downloadcenter')) {
      _menus.push({
    ...
    if (!init || pageDirectorys?.includes('myreward')) {
      _menus.push({
        title: (
          <span style={{ position: 'relative' }}>
    ...
    return _menus;
  }, [isSub, needNotice, myRewardsEntranceClickMarkTime, multiSiteConfig]);

...
import { getTermId, getTermUrl } from 'gbiz-next/tools';
import { useMultiSiteConfig } from 'gbiz-next/hooks';
import { Alert, Box, Button, Checkbox, styled } from '@kux/mui';
  ...
  return (
    <Fragment>
      <Box as="section">
        ...
        {map(tenantConfig.delete.tips(_t), (item) => {
          return <Tip key={item}>{item}</Tip>;
        })}
      </Box>
      <StyledAlert
        showIcon
      ...
      >
        {tenantConfig.delete.showShortNoteMsg ? (
          <span>
            {_tHTML('744a1b890ac04800aaa4', {

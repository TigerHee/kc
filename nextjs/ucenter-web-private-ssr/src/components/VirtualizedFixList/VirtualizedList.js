...
export default class VirtualizedList extends React.Component {
  ...
  getStateFromProps = (props) => {
    ...
    if (hasRecord) {
      ...
      if (
        ...
        /** 扩充缓冲视口大小，最终渲染包含缓冲区域 */
        const bufferHeight =
          (typeof bufferSize === 'undefined' ? VirtualizedList.BUFFER_SIZE : bufferSize) *
          rowHeight;
  ...
  render() {
    ...
    const { hasRecord, listHeight, showHeight, showData, offsetTop, offsetBottom } =
    ...
    return (
      <div ref={this.storeRef} style={_style}>
        <div style={_showStyle}>{showRender(showData)}</div>
      </div>
    );
  }
}

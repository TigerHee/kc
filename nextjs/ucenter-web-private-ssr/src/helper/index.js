...
export const fixWithZero = (v, decimal) => {
  ...
  const valueWithBit = toNumber(v) + toNumber(bit);
...
export const isOpenInWechat = (pUA) => {
  const ua = pUA || (typeof navigator !== 'undefined' ? navigator.userAgent.toLowerCase() : '');
...
export const getUserAgentInfo = () => {
  ...
  const browserArray = {
    IE: typeof window !== 'undefined' && (window.ActiveXObject || 'ActiveXObject' in window), // IE
  ...
  for (var i in browserArray) {
    if (browserArray[i]) {
      ...
      if (i == 'IE') {
        ...
        const mimeTypes = typeof navigator !== 'undefined' ? navigator.mimeTypes : {};
      ...
      browserInfo.type = i;
      browserInfo.versions = isNil(versions) ? '' : parseInt(versions);
    }
  }

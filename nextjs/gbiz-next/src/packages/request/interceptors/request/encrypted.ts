...
import { RequestInterceptor, ResponseInterceptor, ResponseType, RuntimeEnvironment } from '../../types';
/**
 * 接口返回数据
 ...
 * @returns
 */
const encrypt = (data: string, key: Uint8Array, iv: Uint8Array = IV) => gcm(key, iv).encrypt(utf8ToBytes(data));
...
export const encryptRequest: RequestInterceptor = ({ sentry, xVersion, kcStorage }) => {
  function captureEvent(message: string, level: 'info' | 'warn' | 'error' = 'error', tags?: any, extra?: any) {
    try {
      sentry?.captureEvent({
        message,
        level,

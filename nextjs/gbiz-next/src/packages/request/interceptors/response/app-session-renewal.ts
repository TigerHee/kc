...
const appSessionRenewal: ResponseInterceptor = (baseConfig) => {
    ...
    isSupportUserInfoCache = await new Promise((resolve) => {
      JsBridge.open(
        ...
        ({ data: version }: { data: string }) => {
          ...
          const isSupport =
            String(version).localeCompare(
              SUPPORT_USER_INFO_VERSION,
              undefined,
              {
                numeric: true,
                sensitivity: 'base',
    ...
    return new Promise((resolve) => {
      JsBridge.open(
        ...
        (result: any) => {
          ...
          return resolve(
            !(result && result.code && result.data && !result.data.uid)
          )
        }
      )
    })
  ...
  async function bridgeRenewal(url: string) {
    ...
    reLoginPromise = new Promise((resolve, reject) => {
      ...
      //旧版容器需要callback
      JsBridge.open(
        { type: 'func', params: { name: 'refreshAppSession' } },
        ({ code }: any) => {
  ...
  return {
    ...
    priority: 100,
    presetEnvironment: [RuntimeEnvironment.APP],
    supportEnvironment: [RuntimeEnvironment.APP],

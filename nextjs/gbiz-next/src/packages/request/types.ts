...
export interface CreateHttpClientConfig extends CreateAxiosDefaults {
  ...
  /** 禁用指定名称的内置请求拦截器 */
  disableRequestPresetInterceptors?: string[]
  /** 禁用指定名称的内置响应拦截器 */
  disableResponsePresetInterceptors?: string[]
  /** 运行环境 */
...
export type AxiosInterceptor<T> = {
  ...
  /** 支持的运行时环境，不配置表示通用 */
  supportEnvironment?: RuntimeEnvironment[]
  /** 默认预置的运行时环境，不配置都预置 */
  presetEnvironment?: RuntimeEnvironment[]
}
...
export interface RequestExtraConfig {
  ...
  cancelable?: boolean
  /** 是否开启缓存，目前先只在SSR环境支持，后面浏览器有需求再完善 */
  cache?: boolean
  [key: string]: any
}

export type RequestConfigWithExtra = InternalAxiosRequestConfig &
  RequestExtraConfig

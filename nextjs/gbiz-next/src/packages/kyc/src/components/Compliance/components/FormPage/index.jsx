 ...
 * 组装表单页
 */
import { cloneDeep, isArray, isEmpty, isString } from 'lodash-es';
...
import Desc from 'kycCompliance/components/FormPage/UploadFile/Desc';
...
import { Title, DescWrapper, FormGroupTipBox, FormGroupBox, FormGroupItemBox } from './style';
...
export default ({
  ...
  // 组件是否隐藏map
  const [hideComponentMap, setHideComponentMap] = useState({});
  ...
  const onRelations = ({ curMetaCode, allV }) => {
    ...
    if (
      curMetaCode &&
      !componentRelations.some(relationItem => relationItem?.trigger?.some(i => getMetaCodeKey(i) === curMetaCode))
    ) {
      return false;
    }
    ...
    componentRelations.forEach(relationItem => {
      ...
      if (relationType === 1) {
        ...
        if (trigger.every(i => i.metaData === allV[getMetaCodeKey(i)] || (!i.metaData && !allV[getMetaCodeKey(i)]))) {
  ...
  const handleSubmit = () => {
    ...
    validateFields()
      .then(async values => {
        ...
        try {
          const defaultParams = {};
          componentGroups.forEach(groupItem => {
            groupItem?.components?.forEach(({ complianceMetaCode }) => {
              if (complianceMetaCode) {
                defaultParams[complianceMetaCode] = '';
              }
            });
          });
          const { params } = onDealData(values);
          const metaMap = { ...defaultParams, ...params };

          // 联动2的受影响组件隐藏时值采用trigger的值
          flowData?.componentRelations?.forEach(relationItem => {
            const { relationType, trigger, affected } = relationItem;
            if (relationType === 2) {
              const triggerItem = trigger[0] || {};
              const { activeMetaCode, activeMetaData } = triggerItem;
              // 联动2的触发组件是否激活
              const isRelationActive = !activeMetaCode || formData[activeMetaCode] === activeMetaData;
              const triggerMetaCode = getMetaCodeKey(triggerItem);
              const affectedMetaCode = affected[0].metaCode;

              if (isRelationActive && hideComponentMap[affectedMetaCode] && formData[triggerMetaCode]) {
                metaMap[affectedMetaCode] = formData[triggerMetaCode];
              }
            }
          });
      ...
      .catch(errorInfo => {
        ...
        if (errorInfo?.errorFields?.length > 0) {
          const el = document.querySelector(`[data-compliancemetacode="${errorInfo.errorFields[0]?.name?.[0]}"]`);
          if (el) {
            el.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        }
        ...
        >
          {componentGroups.map(groupItem => {
            ...
            const itemUseGroupProps = {
              ...
              componentGroupId,
              componentGroupTitle,
              componentGroupDesc: relationState.relationDescMap[componentGroupId] || componentGroupDesc,
              curFormValue,
              setHideComponentMap,
            };
            ...
            const isAllComponentHide = components.every(
              ({ complianceMetaCode }) => hideComponentMap[complianceMetaCode]
            );

            return (
              <>
                {(componentGroupTitle || componentGroupDesc) && !isAllComponentHide ? (
                  <FormGroupTipBox>
                    {componentGroupTitle ? <div className="componentGroupTitle">{componentGroupTitle}</div> : null}
                    {componentGroupDesc ? (
                      <div className="componentGroupDesc">
                        <Desc desc={componentGroupDesc} ignoreBoxMb />
                      </div>
                    ) : null}
                  </FormGroupTipBox>
                ) : null}
                  ...
                  {components.map((componentItem, index) => {
                    ...
                    return (
                      <FormGroupItemBox
                        ...
                        style={{
                          flexBasis: `calc(${100 / (colsNumConfig[componentGroupTemplateCode] || 1)}% - 16px)`,
                        }}
                        key={complianceMetaCode}
                      >
                        <FormItemRender {...itemUseGroupProps} componentItem={componentItem} key={complianceMetaCode} />
                      </FormGroupItemBox>
                    );
                  })}
                </FormGroupBox>

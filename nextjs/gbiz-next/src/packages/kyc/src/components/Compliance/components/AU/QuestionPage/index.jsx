...
export default ({ onPrePage, pageCode, pageAfterApi, pageId, complianceMetaCodes, onNextPage }) => {
  ...
  // 是否多个题目
  const isMultipleQuestion = curQuestion.showSubOptions && curQuestion.subOptions;
  // 选中有输入框的选项
  const withInputOptionItem = useMemo(() => curQuestion.options?.find(i => i.withInput), [curQuestion]);
  ...
  const isNextDisabled = useMemo(() => {
    ...
    if (!selected || selected.length === 0 || selected.some(i => !i)) return true;
    ...
    if (isMultipleQuestion) {
      return (
        <div className="answerBox">
          <MultipleQuestion
            value={selectData?.[curQuestion.id] || []}
    ...
    if (isSelectType) {
      return (
        <div className="answerBox">
          ...
          />
          {withInputOptionItem && selectData[curQuestion.id]?.includes(withInputOptionItem.optionId) ? (
        ...
        {curQuestion.options?.map(({ title, optionId, withInput, inputTitle }) => {
          ...
          return (
            <Fragment key={optionId}>
              ...
              >
                <span>{title}</span>
                {isActive ? isCheckboxType ? <CheckBoxSelectIcon /> : <RadioSelectIcon /> : <UnSelectIcon />}
              </div>
      ...
      {totalQuestionNum > 0 ? (
        ...
        >
          ...
          >
            ...
            {curQuestion.titleSub ? <div className="desc">{htmlToReactParser.parse(curQuestion.titleSub)}</div> : null}
            {isMultipleQuestion ? null : (
              <div className="tip">{isCheckboxType ? _t('8c9c77ece62e4800adf3') : _t('5b852d7b8fe64000a74f')}</div>
            )}
            {getContent()}
          </QuestionContent>
        </ContentBox>

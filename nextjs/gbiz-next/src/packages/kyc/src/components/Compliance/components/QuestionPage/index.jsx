...
export default ({ onNextPage, onPrePage, pageTitle, pageAfterApi, pageId, complianceMetaCodes, pageCode }) => {
  const { _t } = useLang();
  const [form] = useForm();
  ...
  useEffect(() => {
    setInnerPageElements({ ...pageElements, onHeaderLeft: onPre });
  }, [pageElements, onPre]);
  ...
  // 是否展示答题表单
  const isShowQuestionForm = useMemo(() => !isScoreType || (isScoreType && scoreStep === 1), [isScoreType, scoreStep]);
  // 问题总数
  ...
  }, [pageElements?.pageNextButtonTxt, isShowQuestionForm, totalQuestionNum, completeQuestionNum, _t]);
  // 下一步按钮是否disabled
  const isNextBtnDisable = useMemo(() => {
    ...
    return false;
  }, [isShowQuestionForm, completeQuestionNum, totalQuestionNum, isResultChecked, isScoreType, scoreStep]);
  // 当前等级index
  const curLevelIndex = useMemo(
    () => levels.findIndex(({ range }) => (score - range[0] >= 0 || !range[0]) && (score - range[1] <= 0 || !range[1])),
    [score, levels]
  );
  // 当前等级数据
  ...
  return (
    <Wrapper>
                    ...
                    >
                      <Radio.Group size="small">
                        {options.map(({ title: label, optionId }) => {
                          const isGreen = !isScoreType && userSelectOptionId && answerOptionId.includes(optionId);
                          const isRed =
                            !isScoreType &&
                            userSelectOptionId &&
                  ...
                  <Fragment key={id}>
                    ...
                    >
                      <Checkbox.Group size="small">
                        {options.map(({ title: label, optionId }) => {
                          const isGreen =
                            !isScoreType && userSelectOptionId?.length && answerOptionId.includes(optionId);
                          const isRed =
                            !isScoreType &&
      ...
      {isScoreType && scoreStep === 2 ? (
        ...
        >
          <ScoreResult curLevelIndex={curLevelIndex} curLevelItem={curLevelItem} setResultChecked={setResultChecked} />
        </ContentBox>
      ) : null}

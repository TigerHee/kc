...
import { postJsonWithPrefix, GetComplianceFlow, GetTransactionSettings, getFlowConfig, setSiteType } from './service';
...
const EuResultPage = loadable(() => import('./components/EU/SubmitResultPage'));
// KuCard
const KuCardEeKybHomePage = loadable(() => import('./components/KuCard/EeKybHome'));
const KuCardUploadIdTypePage = loadable(() => import('./components/KuCard/UploadIdType'));
...
const PageObj = {
  ...
  [config.EuResultPageCode]: EuResultPage,
  [config.EUTermsV2PageTemplateCode]: TermsV2Page,
  [config.KuCardEeKybHomePageCode]: KuCardEeKybHomePage,
  [config.KuCardUploadIdTypePageTemplateCode]: KuCardUploadIdTypePage,
  default: FormPage,
};
...
export default ({
          ...
          if (res?.success) {
            ...
            const realPages = cloneDeep(pages)
              ...
              .filter(({ componentGroups }) => {
                ...
                return !componentGroups.every(i => i?.components?.every(j => j.isDisplay === 0));
              });
            ...
            if (pages?.length >= 0) {
              ...
              const defaultPageId = Math.max(
                0,
                realPages.findIndex(i => i.pageConfigs?.isTopPage)
              );
              const isMultipleBranch = realPages.some(i => i.pageConfigs?.isTopPage);

              const lastTimeIndex =
                lastPageId && !isMultipleBranch ? realPages.findIndex(i => i.pageId === lastPageId) + 1 : defaultPageId;

              setCurPage(restart ? defaultPageId : lastTimeIndex);
            }
          }
        })
        .catch(err => {
  ...
  useEffect(() => {
    ...
    if (isInDialog) {
      onSetDialogData({
        ...
        pageElements,
        onPrePage,
        pageConfigs: pageItem?.pageConfigs,
        onPageToFixed,
      });
    }
  }, [
  ...
  // 定位到指定 page index
  const onPageToFixed = useCallback(
    pageCode => {
      const fixedPageIndex = flowData?.pages?.findIndex(i => i.pageCode === pageCode);
      if (fixedPageIndex !== -1) {
        setInnerPageElements({});
        setCurPage(fixedPageIndex);
      }
    },
    [flowData]
  );
  ...
  const onPrePage = useCallback(() => {
    ...
    if (pageItem?.pageConfigs?.prePage) {
      onPageToFixed(pageItem?.pageConfigs?.prePage);
    } else {
      setInnerPageElements({});
      setCurPage(pre => Math.max(pre - 1, 0));
    }
  }, [isFirstPage, pageItem?.pageCode, pageItem?.pageConfigs, onExitFlow]);
  ...
  const { run: onNextPage } = useDebounceFn(
    async (pageStepLength = 1) => {
      ...
      const onNext = () => {
        setNextAction(true);
        if (pageItem?.pageConfigs?.nextPage) {
          onPageToFixed(pageItem?.pageConfigs?.nextPage);
        } else {
          setInnerPageElements({});
      ...
      if (inApp) {
        ...
        const curVersion = isSumsubFlow ? config.SUPPORT_SUMSUB_VERSION : config.SUPPORT_JUMIO_VERSION;
  ...
  // 缓存表单数据
  const onCacheFormData = useCallback((val, isReset = false) => {
    if (isReset) {
      setFormData(val);
    } else {
      setFormData(pre => ({ ...pre, ...val }));
    }
  }, []);
  ...
  return (
    <CommonContext.Provider value={commonData}>
      ...
      >
        ...
        {/* h5 header */}
        {isSmStyle ? (
          <MobileSteps {...pageItem} onPrePage={onPrePage} isLastPage={isLastPage} isFirstPage={isFirstPage} />
        ) : null}
        ...
        {isShowErr ? (
          ...
          <Main
            ...
            data-pageCode={pageItem?.pageCode}
          >

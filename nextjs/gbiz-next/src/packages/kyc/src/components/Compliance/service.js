...
import { post, pull as get, upload } from 'tools/request';
...
const postJson = (url, data, isForm, options) => {
  ...
  return post(url, { ...queryParams, ...data }, false, !isForm, options);
};
...
export async function GetResultData112(param) {
  return postJson(`${prefix}/compliance/dynamic/page/page_112/init`, param);
}

// 上传
export async function fetchUploadFile(param) {
  const queryParams = getQueryParams();
  return upload(`${prefix}/compliance/component/file/upload`, {
...
// 移除工单要素
export async function delMataData(param) {
  return postJson(`${prefix}/compliance/transaction/data/remove`, param);
}

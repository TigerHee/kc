...
const isCurProjectInXgray = (xgray = "", curProjectName) => {
  const projects = Array.isArray(curProjectName)
    ? curProjectName
    : [curProjectName];
  return projects.some(
    (project) => xgray && xgray.toLowerCase().includes(project.toLowerCase())
  );
};
...
const shouldReload = (webGray, webGrayPre, curProjectName) => {
  ...
  const isOnlyOneHasCur =
    !isBothHasCur && (hasCurProject(webGray) || hasCurProject(webGrayPre));

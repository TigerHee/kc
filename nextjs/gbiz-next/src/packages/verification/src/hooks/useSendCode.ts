...
import { SEND_CHANNELS } from '../enums';
import { sendValidationCodeUsingPost } from '../api/risk-validation-center';
import { sendEmailCode, sendPhoneCode } from '../utils/sensors';
...
const set = (operationId: string, state: { done: boolean; value: number } = { done: true, value: 0 }) => {
  const { done = true, value = 0 } = state;
  CD_MAP[operationId] = { done, value };
  emitter.emit(operationId, CD_MAP[operationId]);
};
const get = (operationId: string) => CD_MAP[operationId];
/** 计时开始函数 */
const start = async (operationId: string, _count: number) => {
...
export default function useSendCode({ operationId, bizType, transactionId, sendChannel }) {
  ...
  const sendCode = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const { data, code, msg } = await sendValidationCodeUsingPost(
      ...
      success = true;
      start(operationId, data?.retryAfterSeconds ?? 0);
    } catch (err: any) {
      ...
      setLoading(false);
      const sensorSendCode = sendChannel === SEND_CHANNELS.EMAIL ? sendEmailCode : sendPhoneCode;

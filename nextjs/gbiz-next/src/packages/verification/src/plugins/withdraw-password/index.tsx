...
import { IIconProps, TradingpasswordThinIcon } from '@kux/iconpack';
...
export const Name = (props: React.HTMLAttributes<HTMLSpanElement>) => {
  const { t } = useLang();
  return <span {...props}>{t('71c5a52283604000aae2')}</span>;
};

export const Icon = (props: IIconProps) => {
  return <TradingpasswordThinIcon {...props} />;
};
...
export const VerifyItem = ({ value, onValueChange, onReFocus }) => {
  ...
  return (
    <InnerFormItem label={t('da8288689e3a4000a1f4')} name={field}>
      <Input
        type="password"
        value={value}
        addonAfter={null}

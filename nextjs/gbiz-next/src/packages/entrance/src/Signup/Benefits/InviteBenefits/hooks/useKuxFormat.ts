...
export const NumberFormat = ({ lang, number, isPositive, options = {} }: { lang: string; number: number; isPositive: boolean; options: Intl.NumberFormatOptions }) => {
  try {
    const _lang = (lang || bootConfig._DEFAULT_LANG_).replace('_', '-');
    // 得到带千分位的结果
...
// 只格式化千分位，不国际化格式化
const normalNumberFormat = ({ lang, number, isPositive, options = {} }: { lang: string; number: number; isPositive: boolean; options: Intl.NumberFormatOptions }) => {
  try {
    // 得到带千分位的结果
...
export const numberFormat = ({
  ...
  needAbs = true,
  defaultText = '--',
}: { lang: string; number: number; isPositive: boolean; currency: string; options: Intl.NumberFormatOptions & { _ignoreFractionDigits?: boolean }; needAbs: boolean; defaultText: string }) => {

...
export function useLimitTaskConfig() {
  ...
  const fixedNewCommerConfig = {
    ...
    myApr: parseInt(limitTask?.financialNewcomerTaskInfo.extraApr || '0', 10) || newCommerConfig.myApr,
  };

  const fixedVipConfig = {
    ...vipConfig,
    financialBenefitSubTitle: limitTask?.financialVipTaskInfo?.financialBenefitSubTitle,
    myApr: parseInt(limitTask?.financialVipTaskInfo.extraApr || '0', 10) || vipConfig.myApr,
  };

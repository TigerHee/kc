...
interface ValidateFormProps {
  namespace: string;
  onSuccess?: (data: TUserResponse) => void;
...
const ValidateForm: React.FC<ValidateFormProps> = props => {
  ...
  // 用户资产申领协议
  const withdrawPolicyUrl = addLangToPath(`${getSiteConfig()?.KC_SITE_HOST}/announcement/en-assets-recovery-agreement`);
  ...
  const canSubmit = useMemo(() => {
    ...
    return true;
  }, [policyCheck, hasEmail, inputEmailCode.error, hasSMS, inputPhoneCode.error, hasGFA, inputG2FACode.error]);
              ...
              <div className={styles.agreeLabel} onClick={e => e.stopPropagation()}>
                ...
                >
                  <Trans
                    ...
                    components={{
                      a: <a href={withdrawPolicyUrl} target="_blank" rel="noopener noreferrer" />,
                    }}
                  />
                </Checkbox>
              </div>

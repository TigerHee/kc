...
function mergeData(data: any, cacheData: any): any {
...
export default function useCurrenciesFetch(options: any): any {
  const {
    manual,
    onError,
    onSuccess,
  ...
  const { data, loading, run } = useRequest(
    async (...rest: any[]) => {
      const res = await getCurrencies(rest[0]);
      return res?.data || {};
    },
    {
      ...DEFAULT_OPTIONS,
      ...
      formatParams: async () => {
        ...
        if (cacheData && Object.hasOwnProperty.call(cacheData, 'time')) {
          domainIds = domainIdsFromParams.filter((domainId: any) => {
            return !cacheData.time?.[domainId] || !checkIsStale(staleTime, cacheData.time[domainId]);
          });
        } else {
        ...
        domainIds.forEach((domainId: any) => {
          ...
          if (DEFAULT_DOMAINIDS.includes(domainId)) {
            conditions.push({
              timeline,
              domainId,
              currencyType: 2,
      ...
      setCache: async ({ time, data }: any) => {
        const cacheData = cache.current || {};
        cacheData.timeline = { ...cacheData.timeline, ...data.timeline };

        Object.keys(data.currencies).forEach((domainId: any) => {
      ...
      getCache: async (reqParams: any) => {
        const cacheData = cache.current;
        if (!cacheData) return null;

        const currencies = domainIdsFromParams.reduce((acc: any, domainId: any) => {
  ...
  useEffect(() => {
    if (data?.currencies) {
      const newData = domainIdsFromParams.reduce((acc: any, domainId: any) => {
        if (data.currencies[domainId]) {

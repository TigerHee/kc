...
import { Config, DriverFuncType } from './types';
...
const DefinedDrivers: any = {};
const DriverSupport: any = {};
...
const DefaultDriverOrder: Array<string> = [
...
function callWhenReady(storageInstance: any, libraryMethod: string) {
...
class KcStorage {
  ...
  config(options?: Config) {
    ...
    const validateAndSanitizeOptions = (key: string, value: string) => {
      if (key === 'storeName') {
  ...
  defineDriver = (driverObject: any, callback?: any, errorCallback?: any) => {
    const promise = new Promise((resolve, reject) => {
      try {
        const driverName = driverObject._driver;
        const complianceError = new Error('Custom driver not compliant; Please check function config');
        ...
        const configureMissingMethods = (...args: any) => {
  ...
  getDriver = (driverName: string | number, callback?: any, errorCallback?: any) => {
  ...
  setDriver(drivers?: any, callback?: any, errorCallback?: any) {
    ...
    const initDriver = supports => {
      ...
      const driverPromiseLoop = () => {
        ...
        return thisInstance.getDriver(driverName).then(extendSelfWithDriver).catch(driverPromiseLoop);
      };
      return driverPromiseLoop();
    };
    const oldDriverSetDone = this._driverSet ? this._driverSet.catch(() => Promise.resolve()) : Promise.resolve();
  ...
  supports = (driverName: string | number) => {
    return !!DriverSupport[driverName];
  };

  _extend(libraryMethodsAndProperties: DriverFuncType) {
    extend(this, libraryMethodsAndProperties);
  }

  _getSupportedDrivers(drivers: any) {

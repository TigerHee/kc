...
const BLOB_TYPE_PREFIX = '~~kc_storage_type~';
const BLOB_TYPE_PREFIX_REGEX = /^~~kc_storage_type~([^~]+)~/;
...
function stringToBuffer(serializedString: string | any[]) {
...
function deserialize(value: string) {
  ...
  // DBs created with older versions will simply not have the blob type.
  if (type === TYPE_BLOB && BLOB_TYPE_PREFIX_REGEX.test(serializedString)) {
    const matcher: any = serializedString.match(BLOB_TYPE_PREFIX_REGEX);

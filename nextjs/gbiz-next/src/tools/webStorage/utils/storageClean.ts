...
export const _legacyStorageCleanup = (type, prefix) => {
  ...
  // 需要遍历后先收集所有key再删除
  const keys: any = [];
  const windowType: any = window[type];

  if(!windowType)return keys;

  for (let i = 0; i < windowType.length; i++) {
    const k = windowType?.key(i);
...
export const _retryOperation = ({ type, key, saveValue, reporter, resolve, reject }) => {
  try {
    const windowType: any = window[type];
    // 先尝试直接存储
    windowType?.setItem(key, saveValue);
    resolve(saveValue);
  } catch (retryError) {

 ...
 */
import { DbInfo, DeferredOperation, DriverFuncType } from '../types';
...
const dbContexts: { [key: string]: any } = {};
...
// Transform a binary string to an array buffer
function _binStringToArrayBuffer(bin: string): ArrayBuffer {
...
// Check blob support without caching
function _checkBlobSupportWithoutCaching(idb: IDBDatabase): Promise<any> {
  return new Promise(resolve => {
    ...
    txn.onabort = (e: Event) => {
      // If the transaction aborts now it's due to not being able to
    ...
    txn.oncomplete = () => {
      if (typeof navigator !== 'undefined') {
        ...
        // If `navigator` is not available (SSR environment), assume no blob support
        resolve(false);
      }
    };
  }).catch(() => {
...
function _checkBlobSupport(idb: IDBDatabase): Promise<any> {
...
function _deferReadiness(dbInfo: { name: string }): void {
  const dbContext = dbContexts[dbInfo.name];

  // Create a deferred object representing the current database operation.
  const deferredOperation: DeferredOperation = {} as DeferredOperation;

  deferredOperation.promise = new Promise<void>((resolve, reject) => {
...
function _advanceReadiness(dbInfo: DbInfo): Promise<void> | null {
  const dbContext = dbContexts[dbInfo.name];
  // Dequeue a deferred operation.
  const deferredOperation: DeferredOperation = dbContext.deferredOperations.pop();
...
function _rejectReadiness(dbInfo: DbInfo, err: any): Promise<void> | null {
  const dbContext = dbContexts[dbInfo.name];
  // Dequeue a deferred operation.
  const deferredOperation: DeferredOperation = dbContext.deferredOperations.pop();
...
function _isUpgradeNeeded(dbInfo: DbInfo, defaultVersion?: string) {
  if (!dbInfo.db) {
    return true;
  }
...
function _decodeBlob(encodedBlob: { data: string; type: any }) {
  const arrayBuff = _binStringToArrayBuffer(atob(encodedBlob.data));
  return createBlob([arrayBuff], { type: encodedBlob.type });
}
// is this one of our fancy encoded blobs?
function _isEncodedBlob(value: { __local_encoded_blob: any }) {
...
function _fullyReady(this: any, callback: any) {
  const self = this;
...
function createTransaction(dbInfo: DbInfo,
  mode: string,
  callback: any,
...
function getItem(this: DriverFuncType, getKey: any, callback: any) {
  const self = this;
...
function setItem(this: DriverFuncType,
  setKey: any,
  value: Blob,
  callback: any) {
  const self = this;
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    if (checkStrLength(value, self._dbInfo?.maxItemSize)) {
      sentryReport(`setItem maxLength`, `set key ${setKey} value: ${!!value} maxItem: ${self._dbInfo?.maxItemSize}`);
      return reject(`The single storage limit does not exceed ${self._dbInfo?.maxItemSize || 150 * 1024} chart!`);
    }
    let dbInfo;
    self
      .ready()
      .then(() => {
        ...
        if (toString.call(value) === '[object Blob]') {
          console.warn('Using blob for storage is not recommended and may result in reduced performance');
          // save blob warning
...
function keys(this: any, callback: any) {
  const self = this;
  const promise = new Promise((resolve, reject) => {
    self
      .ready()
...
// store clear
function clear(this: any, callback: any) {
  const self = this;
  const promise = new Promise((resolve, reject) => {
    self
      .ready()

...
import { getSitCustomConfigUsingGet, SiteConfig, TermConfig } from 'packages/entrance/src/api/ucenter';
import { useState, useEffect } from 'react';
import { safeJSONParse } from 'tools/helper';
import storage from 'tools/storage';
import { bootConfig } from 'kc-next/boot';

type ToLowercase<T> = T extends string ? Lowercase<T> : T;

type DeepToLowercase<T> = T extends Array<infer U>
  ? Array<DeepToLowercase<U>>
  : T extends object
    ? { [K in keyof T]: DeepToLowercase<T[K]> }
    : ToLowercase<T>;

export type TSiteConfig =  DeepToLowercase<Omit<SiteConfig, 'termConfig'>> & { termConfig: TermConfig };
type TMultiSiteConfig = { KC: TSiteConfig; TR: TSiteConfig; TH: TSiteConfig; AU: TSiteConfig; EU: TSiteConfig; CL: TSiteConfig };
...
export const MULTI_SITE_CONFIG: TMultiSiteConfig = {
  KC: {
    accountConfig: {
    ...
    myConfig: {
      ...
      directoryConfig: {
        ...
        pageDirectorys: ['overview', 'accountsecurity', 'kyc', 'api', 'downloadcenter', 'myreward', 'subuser'], // 目录配置
        // 头像下拉目录 accountinfo、 ratestandard、 accountsecurity、 kyc、 api、 subuser
        profilePhotoDirectorys: ['accountinfo', 'ratestandard', 'accountsecurity', 'kyc', 'api', 'myreward', 'subuser'],
        // 费率跳转url
  ...
  TR: {
    ...
    myConfig: {
      directoryConfig: {
        ...
        profilePhotoDirectorys: ['accountinfo', 'myreward'],
  ...
  AU: {
    ...
    myConfig: {
      directoryConfig: {
        pageDirectorys: ['overview', 'accountsecurity', 'kyc', 'api', 'downloadcenter', 'myreward', 'subuser'],
        profilePhotoDirectorys: ['accountinfo', 'ratestandard', 'accountsecurity', 'kyc', 'api', 'myreward', 'subuser'],
        rateStandardUrl: '/vip/privilege',
      },
      overviewConfig: {
        ...
        supportVipRate: true,
        assetFuncs: ['assetoverview', 'entrustedinquiry', 'depositcoins', 'withdrawalcoins', 'buycoins'],
...
let targetMultiSiteConfig: TSiteConfig | null = null;
...
const getMultiSiteConfig = async () => {
  ...
  // 2:如果是本地站，则发起请求
  currentPromise = getSitCustomConfigUsingGet()
    .then((res) => {
      const { data, success } = res;
      if (success && data) {
        targetMultiSiteConfig = data as TSiteConfig;
    ...
    .catch(() => {
      ...
      // 4.如果没有缓存，则使用默认的配置
      targetMultiSiteConfig = MULTI_SITE_CONFIG[bootConfig._BRAND_SITE_] || MULTI_SITE_CONFIG.KC;
      return MULTI_SITE_CONFIG[bootConfig._BRAND_SITE_] || MULTI_SITE_CONFIG.KC;
    });
  return currentPromise;
};

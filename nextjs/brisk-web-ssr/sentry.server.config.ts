...
Sentry.init({
  ...
  beforeSend: (event, hint) => {
    try {
      ...
      console.warn('Sentry server | 捕获到错误事件:', event, event?.message, stackText);
    } catch (error) {
      console.warn('Sentry server | event解析失败', (error as any)?.message);
...
// 初始化后设置全局extra
Sentry.setExtra('runtimeEnv', 'server');

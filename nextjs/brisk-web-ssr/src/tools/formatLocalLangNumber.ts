 ...
 *
 */
const numSeparateDecimal = (num: any, interceptDigits = 2) => {
 ...
 */
export default ({ lang = 'en_US', data, options = {}, interceptDigits = null }: any) => {
  const num = +data;
  ...
  const result = numberFormat.format(newNum);
  return result;
};

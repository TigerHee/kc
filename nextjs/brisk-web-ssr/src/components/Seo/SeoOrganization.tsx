...
const jsonLd = {
  ...
  alternateName: ['庫幣', 'クーコイン', '쿠코인'],
  description:
    'KuCoin is a secure cryptocurrency exchange that allows you to buy, sell, and trade Bitcoin, Ethereum, and 1300+ trading pairs',
  foundingDate: '2017',
  founder: [
    {
      '@type': 'Person',
...
interface SeoOrganizationProps {
  currentLang: string;
  pathname?: string;
}

// 只配置了主站英文首页
const SeoOrganization: React.FC<SeoOrganizationProps> = ({
  currentLang,
  pathname,
}) => {
  ...
  if (
    currentLang !== 'en_US' ||
    pathname !== '/' ||
    !tenantConfig.enableSeoOrganization
  ) {
    return null;
  }
  ...
  return (
    <script
      ...
      data-inspector="seo-organization"

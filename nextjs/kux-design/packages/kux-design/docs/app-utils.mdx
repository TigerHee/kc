...
<Meta title="7. 全局工具对象 app" />

# 全局工具对象 app

kux-design 依赖了工具库 @kux/app-sdk, 其中提供了全局工具对象 app, 可以在项目中直接使用。
...
app.config({
  ...
  timeZone: 'UTC+8', // 前端默认使用 utf8 时区格式化时间
})
...
// 写入到内存, 并使用自定义存储的前缀
app.storage({area: 'memory', namespace: 'kux'}, 'version', '1.1.1')

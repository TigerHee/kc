...
/**
 * 命令行选择需要打包的packages，并写入.publish-packages文件
 */
...
function getPkgs() {
  ...
  return files.filter(file => {
    if (!file.isDirectory() || excludePkg.includes(file.name)) return false;
    const pkgPath = path.join(folderPath, file.name, 'package.json');
...
async function version() {
  ...
  const spinner = ora('Upgrade .publish-packages...').start();
  if (!projects.length) {
    spinner.stop();
  ...
  try {
    execa('rm -rf .publish-packages');
    const fWritePath = path.join(cwd(), '.publish-packages');
    projects.forEach(p => {
      fs.appendFileSync(fWritePath, `${p},`);
    })
  } catch (e) {
    console.log(e);
    execa('rimraf .publish-packages');
    spinner.stop();
  ...
  await git.add('.');
  // 生成发版commit log
  await git.commit('feat(t-ways-stop): prepare versions');
